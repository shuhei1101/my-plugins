...
| --- | --- | --- | --- | --- | --- | --- |
| `INJECT_RULES_INDEXES` | string | - | なし（何もせず終了） | ルール索引ファイルの URL のカンマ区切りリスト | - | 先頭の索引が優先。索引の置き場所は問わない |
...
| --- | --- | --- | --- | --- | --- | --- |
| `rules` | object[] | ✅ | - | ルールの一覧 | - | - |
| `rules[].url` | string | ✅ | - | ルール本文の絶対 URL | 絶対 URL のみ | 索引の置き場所と無関係にルールを指せる |
...
```yaml
rules:
  - url: https://raw.githubusercontent.com/shuhei1101/ai-monitor/master/docs/wiki/規約/Wiki管理.md
    paths:
      - "**/docs/wiki/**"
  - url: https://raw.githubusercontent.com/shuhei1101/my-plugins/master/docs/wiki/rules/python/core/命名規則.md
    paths:
      - "**/*.py"
```
...
| 取得のタイムアウト | 10 秒 | 索引・ルール本文とも同じ |
| glob の記法 | 索引の `paths` はクォート必須 | クォートなしの値は無視する |
| ルール本文の指定 | 索引の `url` は絶対 URL で書く | 索引の置き場所とルールの置き場所を独立させる |
| 取得元の公開範囲 | 認証なしで取得できる URL であること | raw 配信は private リポジトリだと 404 になる |
...
- 各ブロックの取得元 URL がそれぞれの索引から解決された URL になっている
- `permissionDecision` が `allow`
