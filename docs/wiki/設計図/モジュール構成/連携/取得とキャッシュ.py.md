...
`連携` ドメインに属する構成要素詳細。
外部への出入り（場所からのテキスト取得、ローカルキャッシュ、観測基盤へのログ送出）と、その設定を扱う。
...
| --- | --- | --- | --- | --- | --- | --- |
| ルール注入 | 設定 | `shared/settings.py` | データモデル | [`設定`](#設定) | 索引の場所一覧・送信先・保存先 | 環境変数から読む |
| ルール注入 | 型 | `shared/types.py` | 関数型 | [`テキスト取得関数`](#テキスト取得関数) | 場所から本文を得る関数の型 | 差し替え点 |
| ルール注入 | ログ | `shared/logger.py` | 関数 | [`ログ送出`](#ログ送出) | 観測基盤へ 1 件送る | SDK 未導入なら何もしない |
| ルール注入 | 取得 | `integrations/http/fetcher.py` | 関数 | [`テキスト取得`](#テキスト取得) | 場所から本文を取得する | リモートは HTTP・ローカルはファイル読み |
...
| ルール注入 | キャッシュ | `integrations/cache/store.py` | 関数 | [`キャッシュ保存`](#キャッシュ保存) | 本文をキャッシュへ書く | 場所単位 |
...
キャッシュは場所を鍵にしたフラットな構造にする。
索引ごとにディレクトリを分ける必要はなく、場所が絶対なので衝突しない。
...
classDiagram
  ...
  class キャッシュ取得 {
    <<function>>
    +キャッシュ取得(場所, キャッシュディレクトリ, 寿命秒) str | None
  }
  class キャッシュ保存 {
    <<function>>
    +キャッシュ保存(場所, 本文, キャッシュディレクトリ) None
  }
  class キャッシュ付き取得 {
    <<function>>
    +キャッシュ付き取得(場所, 取得関数, キャッシュディレクトリ, 寿命秒) str
  }
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 索引の場所一覧 | `index_locations` | `tuple[str, ...]` | 公開 | `()` | ルール索引の raw URL またはローカル絶対パス | `("https://example.com/docs/rules.yaml", "/home/user/repo/my-plugins/docs/rules.yaml")` | 環境変数 `INJECT_RULES_INDEXES` をカンマで分割 |
...
```python
Settings(
    index_locations=("https://example.com/docs/rules.yaml",),
...
| `test_from_env_when_unset` | 正常 | 未設定時の既定値 | 全変数を削除 | monkeypatch.delenv | `index_locations` が空・他は既定値 | - |
...
場所から本文を得る関数の型（`type FetchText = Callable[[str], str]`）。
...
| 属性 | `attributes` | `dict[str, str]` | - | `{}` | 付随情報 | 索引の場所など |
| 送信先 | `endpoint` | `str` | ✅ | - | OTLP gRPC の受付 URL | キーワード引数 |

引数例:

```python
emit_log("WARNING", "索引を取得できませんでした", {"index_location": location}, endpoint=settings.otlp_endpoint)
```
...
`http://` / `https://` で始まる場所は HTTP で取得し、それ以外はローカルのファイルとして読む。

#### 引数

| 論理名 | 引数名 | 型 | 必須 | デフォルト | 説明 | 補足 |
| --- | --- | --- | --- | --- | --- | --- |
| 場所 | `location` | `str` | ✅ | - | 取得先の raw URL またはローカル絶対パス | - |
| タイムアウト | `timeout` | `int` | - | `10` | 取得のタイムアウト秒数 | キーワード引数。ローカル読みでは使わない |

引数例:

```python
fetch_text("https://raw.githubusercontent.com/owner/repo/master/docs/rules.yaml")
fetch_text("/home/user/repo/my-plugins/docs/rules.yaml")
```
...
1. 場所の種類で取得手段を決める
   - `http://` / `https://` で始まらない場合、UTF-8 でファイルを読み、先頭の YAML フロントマターを取り除いて返す
   - それ以外の場合、2 以降で HTTP 取得する
2. [URL正規化](#url正規化) で取得先を決める
3. URL の非 ASCII 文字をパーセントエンコードする
4. HTTP GET で本文を取得し、応答の文字コードで復号する
5. 応答の種類に応じて本文を整形して返す
...
| `OSError` | ローカルのファイルが存在しない / 読めない | 標準ライブラリのメッセージ | 同上（`URLError` は `OSError` の派生） |
...
| `test_fetch_text_when_local_path` | 正常 | ローカルファイルの読み込み | 一時ファイルの絶対パス | urlopen | 本文が返り、urlopen が呼ばれない | - |
| `test_fetch_text_when_local_front_matter` | 正常 | ローカルのフロントマター除去 | 先頭にフロントマターを持つ一時ファイル | urlopen | フロントマターが落ちる | raw 配信と同じ扱い |
| `test_fetch_text_when_unreachable` | 異常 | 取得失敗 | urlopen が `URLError` | urlopen | `URLError` が伝播する | - |
| `test_fetch_text_when_local_missing` | 異常 | ローカルファイル不在 | 存在しない絶対パス | urlopen | `OSError` が伝播する | - |
...
| `test_read_cache_when_missing` | 正常 | 未取得 | 未保存の場所 | なし | `None` | - |
...
2. 場所のハッシュから求めたパスへ本文を書き出す（書き出しに失敗した場合は握りつぶす）
...
索引もルール本文も本関数を通す。
ローカルの場所はキャッシュせず、毎回ファイルを読む（編集をそのまま反映させるため）。
...
1. 場所が `http://` / `https://` で始まらない場合、取得関数の結果をそのまま返す（キャッシュを読み書きしない）
2. [キャッシュ取得](#キャッシュ取得) で有効期限内のキャッシュを探す
   - 見つかった場合、そのまま返す（取得関数は呼ばない）
3. 取得関数で本文を取得する
   - 失敗した場合、期限を問わずキャッシュを探す
     - 見つかった場合、期限を延ばして継続利用し、その本文を返す
       - `[WARNING]` 取得に失敗したのでキャッシュを継続利用する（場所 / 例外）
     - 見つからない場合、例外をそのまま伝播させる
4. [キャッシュ保存](#キャッシュ保存) で保存してから本文を返す
...
| `test_fetch_with_cache_when_local` | 正常 | ローカルはキャッシュしない | 場所がローカル絶対パス | 取得関数 | 毎回取得関数が呼ばれ、キャッシュが作られない | 編集をそのまま反映する要件 |
