...
| 26 | Git / branch / merge workflow | migration-reruns-resets-processed-state | Re-running a batch-transformation script on already-processed files silently resets their state (e.g. `[x]` checkboxes → `[ ]`). After a merge that introduces new files, apply the script only to those new files — or verify idempotency first. |
