...
| 2026-05-24 | `disable-model-invocation: true` のスキルに「ユーザーの明示的指示なしで実行するな」系の禁止事項を多数書いていた。このフラグがあればスキルはユーザーの直接呼び出し以外では発動しないため全て冗長。直近メッセージでの呼び出しチェックのみ残せばよい | [detail](../../references/incidents/disable-model-invocation-redundant-prohibitions.md) |
