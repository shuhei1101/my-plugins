...
| py-kit | dev-kit から Python を分離した新規プラグイン（PR129）。当初は純DDD（domain / application / infrastructure / interface）を標準レイアウトに採用したが、**PR138 で「機能フォルダ型レイアウト + TypeScript 風 + 関数ファースト」へ大方針転換**。トップレベルは `src/{pkg}/{shared,features,integrations,runtime,server}/` 構成（`shared/` と `main.py` のみ必須）、DTO は用途別に Pydantic / @dataclass / TypedDict、振る舞いはモジュールレベルの関数、TypeScript 風の抽象化は `type` エイリアス・`Callable[...]`・`Protocol`・`@overload` で実現。DB は対象外（Web は next-kit に委譲）。新レイアウトの実装は後続 PR（`rebuild-py-kit-references`）で予約。 |
