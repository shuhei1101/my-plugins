...
PR163 で worktree-kit プラグインを work-kit に統合した。worktree 関連の 2 スキル
（`work-add` / `vscode-workspace-sync`）は work-kit 配下にある。
`work-start` の Step 4 は `work-kit:work-add` に委譲しているため、両者のインターフェースを揃える必要がある。

## ファイル依存関係

| 編集ファイル | 合わせて確認・更新 |
|---|---|
| `plugins/work-kit/skills/work-start/SKILL.md` | `plugins/work-kit/skills/work-add/SKILL.md` — インターフェース（PR番号・ブランチ引数）が一致しているか確認 |
| `plugins/work-kit/skills/work-add/SKILL.md` | `plugins/work-kit/skills/work-start/SKILL.md` — Step 4 の呼び出し形式と一致しているか確認 |
| `plugins/work-kit/skills/vscode-workspace-sync/SKILL.md` | ネームスペースが `work-kit:` であることを確認 |
