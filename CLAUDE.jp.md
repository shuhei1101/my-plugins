...
- **SKILL.md 一ファイルに全内容を入れる** — `references/` 等の外部ファイルは作らない（トリガーのたびに読み込みのレイテンシが発生するため）
  - **例外：`rules/` サブフォルダ** — プロジェクト初期化時にデプロイするルールファイルテンプレートは `SKILL.md` と並置する `rules/` フォルダに置いてもよい。通常のスキル実行中には読まれない。`SKILL.md` にはテンプレート内容をインラインで埋め込むこと（ファイル検索なしで動けるように）。`rules/` は人間が参照するソースオブトゥルース。
...
| プラグイン | バージョン | 説明 |
|-----------|-----------|------|
| `docs-manage` | 1.2.0 | Issue 駆動の意思決定追跡を含むプロジェクトドキュメント管理 |
| `py` | 1.1.0 | Python プロジェクトのコーディング規約 |
| `wt` | 1.1.0 | Git worktree を使った実装ワークフロー管理 |
| `claude-kit` | 3.2.0 | CLAUDE.md・パス別ルール・スキルの記述ツールキット |
| `yaml-rule` | 1.0.0 | アセット・プロジェクト設定の YAML 管理規約（index.yaml / settings.yaml / 開発者ノート） |
| `work-kit` | 2.0.0 | フックベースのプロジェクトライフサイクル管理（PR タスク状況の context 注入・タスク更新リマインド） |
