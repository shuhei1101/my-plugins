...
`_common.py` の `already_dispatched_this_session()` は以下の実装:
...
`flag.exists()` の True 確認と `flag.touch()` の間に別プロセスが同じフラグファイルを作成できる（TOCTOU: Time-Of-Check to Time-Of-Use）。Claude Code が複数の並列エージェントや並列フックを起動する場合（例: `work:issue-scan` が複数の `issue-scanner` サブエージェントを同時実行するとき）、両プロセスが `flag.exists()` を `False` と判定して両方とも `False` を返す可能性がある。

結果として、セッション単位で「1 回だけ発火」を保証したい guard フック（`references_edit_guard.py` 等）が同一セッションで複数回発火し、重複した `decision: block` インジェクションが発生する。ユーザーへの直接的なセッション破壊は起きないが、同じプロンプトが複数回注入されてコンテキストが汚染される。
