...
| No | 完了 | 作業内容 |
|---|---|---|
| 1 | - | `reload_plugins.py` で自セッションを検出し、保留トークン（`tokens/work/reload-pending/`）を書く |
| 2 | - | Stop フック `reload_deferred.py` を新規作成（保留トークン消費 → 遅延 send-keys） |
| 3 | - | `hooks.json` に Stop エントリを追加 |
| 4 | - | work プラグインのバージョンバンプ |
...
| No | 確認内容 | 実測結果 | 判定 |
|---|---|---|---|
| 1 | 自セッション検出が tmux 内で正しく動く | (未実施) | - |
| 2 | reload_deferred が保留トークンを消費して遅延送信する | (未実施) | - |
| 3 | 保留トークンがないとき何もしない | (未実施) | - |
