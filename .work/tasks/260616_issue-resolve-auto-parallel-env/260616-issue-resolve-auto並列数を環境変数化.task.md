...
| No | 完了 | 作業内容 |
|---|---|---|
| 1 | 済 | スキル冒頭に環境変数読み込みセクションを追加（`!`echo` バッククォート構文） |
| 2 | 済 | Step 1 / Step 5 の固定値「5」を環境変数参照に置き換える |
| 3 | 済 | `marketplace.json` / `plugin.json` の version bump |
...
| No | 確認内容 | 実測結果 | 判定 |
|---|---|---|---|
| 1 | スキル本文を読み環境変数参照が固定値「5」から差し替わっていること | 「最大 5 件」「上から 5 つ」が **N** に置換、冒頭に環境変数セクション追加を確認 | ✅ |
