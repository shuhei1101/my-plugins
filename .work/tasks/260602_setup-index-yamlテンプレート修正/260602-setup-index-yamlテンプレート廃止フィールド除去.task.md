...
| 2 | - | `_TASKS_INDEX_YAML`: `last_id: 0` を除去し、コメントを現行スキーマ（branch/created/title/type/summary/task/completed）に更新 |
| 3 | - | `_TASKS_INDEX_ARCHIVE_YAML`: コメントを `cmd_archive` が実際に出力するフィールドに合わせて更新（`id`, `tags`, 旧 `resolution` 表記を除去） |
| 4 | - | 構文検証・YAML パース検証 |
...
| # | 確認内容 | 実測結果 | 判定 |
|---|---|---|---|
| 1 | `setup.py` が構文エラーなくパースできる | (未実施) | - |
| 2 | `_TASKS_INDEX_YAML` 定数が有効な YAML としてパースできる | (未実施) | - |
| 3 | `_TASKS_INDEX_ARCHIVE_YAML` 定数が有効な YAML としてパースできる | (未実施) | - |
| 4 | 生成された `index.yaml` に `last_id` フィールドが含まれない | (未実施) | - |
