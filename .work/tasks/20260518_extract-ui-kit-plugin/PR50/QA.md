...
**状態**: 一部決定済み(2026-05-18)

### 決定済み

- スキル名: `ui-kit:mock`
- 出力形式: 単一 HTML ファイル(タブで案を切り替え)
- タブ位置: 画面最上部、案数は内容次第(目安 A〜E)
- 画面下部にモック本体を表示
- PC + モバイル両対応(レスポンシブ)

### 未決定

- 出力先パス: ユーザーのプロジェクト内 `tmp/mocks/` か、別の場所か
- モック対応の画面タイプ:
  - **設定画面**(✅ 含める)
  - **一覧 + 詳細画面**(✅ 含める、PC は 2 ペイン、モバイルは画面遷移)
  - **トップ画面**(✅ サイドメニュー + ツールカード列)
  - テーブル画面 ❌ 不要
- 1 スキル実行で複数画面タイプを混ぜて生成するか、画面 1 枚ずつか
