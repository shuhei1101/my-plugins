...
debug-fab に「要素ピッカー」モードを追加:

- モーダルヘッダに「🎯 要素選択」ボタン追加
- 押下 → モーダル一時非表示 + 画面全体にピッカーモード(ホバー要素にアウトライン)
- 任意の要素をクリック → JSON で `{ page, url, element: { xpath, selector, tag, text }, capturedAt }` をクリップボードへコピー
- ピッカーモード終了、モーダル再表示(コピー完了トースト)
- XPath 形式は設定で切替: **full**(`/html/body/...`)または **short**(直近 ID 起点)
- `Esc` でピッカーモード中止

設定はモーダル内に追加(localStorage 保存)、デフォルトは short。

---

## QA-022: 画面切替を URL クエリストリングに反映する必須ルール

**状態**: 決定済み(2026-05-18)

タブ・サイドバーメニュー・ペイン切替など、**画面切替系の状態はすべて URL クエリストリングに反映する**:

- 例: `?tab=settings`、`?view=detail&id=42`、`?nav=tools`
- ユーザーが Claude Code に URL を渡すだけで「どこにいるか」が伝わる
- ブラウザの戻る/進むでも状態が復元される
- 共有可能・ブックマーク可能

実装側のロジックは共通モジュール化を推奨(`static/js/url-state.js` 等)。
ただし「クエリストリング反映」自体は例外なく必須。

principles.md と ui-design.md の両方に明記する。
