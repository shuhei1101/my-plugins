...
| next-kit に references 自動注入フックを追加 | 編集対象ファイルに応じて対応する references を Claude のコンテキストに自動注入するフックを `plugins/next-kit/hooks/` に追加。例: `form.ts` を触ったら `references/frontend/patterns/form.md` を読ませる、`_hooks/use*.ts` を触ったら `references/frontend/hooks.md` を読ませる。`plugins/dev-kit/hooks/hooks.json` が設計の参考。ファイルパス → 注入する reference の対応表を設計し、PreToolUse / UserPromptSubmit のいずれが適切か検討する。 | PR132 完了後（references が整備済みであること） |
