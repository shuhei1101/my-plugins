...
| No | 項目 | 内容 | 完了 |
| -- | ---- | ---- | ---- |
| 1 | 進捗表示の統一 | 1万文字に収まる場合でも「N/M ファイル」を表示する（現状は超過時のみ表示） | |
| 2 | 大ファイル分割読み込み | 1件で1万文字超えるときトークンにオフセットを保存し、次回呼び出しで続きを読む | |
| 3 | TOKEN_DIR バグ修正 | work版が `tokens/dev-kit/rules/` を参照しているのを `tokens/work/rules/` に修正 | |
