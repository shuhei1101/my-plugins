...
flowchart TD
  ...
  WIP -->|/gh:issue-resolve| Ready[(Ready PR + auto-review)]
...
| 3 | `/gh:pr-wip-create` | `go` ラベル付き Issue から Draft PR を 1 つ作成（1 Issue 複数派生可） |
| 4 | `/gh:issue-resolve` | 1 Draft PR を拾い実装 → Ready 化 |
| 5 | `/gh:issue-resolve-auto` | `wip` Draft PR を N 件並列実装 → Ready 化 |
| 6 | `/gh:pr-review-auto` | `auto-review` Ready PR を直列でレビュー → 合格ならマージ |
...
| 4 | `issue-resolver` | `/gh:issue-resolve` / `issue-resolve-auto` | 既存 Draft PR に実装コミットを積み Ready 化 |
...
| `wip` | Draft PR | `pr-wip-create` | `issue-resolve` 完了時 |
| `resolving` | 実装中（排他） | `issue-resolve-auto` | 完了時 |
...
- `plugins/gh/skills/`: 6 スキルの SKILL.md
