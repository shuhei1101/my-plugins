...
- `references/` はフラット構造（直下にトピック別 `.md` を並べる）。スキルからの参照パスが単純になるため。
- Python 系スキルは 2 つ:
  - `py-project` — 本格プロジェクト（DDD レイヤー・src レイアウト・テスト構造を含む）
  - `py-script` — 単一 / 数ファイルの小規模スクリプト（プロジェクト構造は作らない）
- スキルは実行ステップ（手順・トリガー）を定義し、`references/{topic}.md` が規約・知識を定義する。

## Python 規約

- 型ヒントは全箇所に付ける。`Protocol` でインターフェースを定義する。
- SOLID / DRY / DDD（エンティティ・値オブジェクト・集約・リポジトリ・サービス）。
- 拡張性重視: Strategy / Factory / DI パターン。
...
## 生成されるルール（`.claude/rules/`）

- クラス構造（継承ツリー内の波及確認）
- 設定 ↔ ソース紐付け（設定ファイルとそれを読むソースの同期確認）
- ソース ↔ テスト紐付け（`src/` モジュールと `tests/` の更新確認）

## 変更履歴

| # | 日付 | 変更内容 | 関連タスク |
|---|---|---|---|
| 1 | 260531 | dev-kit 現行設計・Python 規約・テスト方針を specs から追記 | 260531_notes-spec-and-ref-inject |
