...
## イシューファイルフォーマット（fix/issue-skills-improve で改訂）

変更: 2026-05-31

### 変更前
- ファイル本文に Type・Priority・Tags・Scan scope を記載
- セクション名が英語（`## Problem`, `## Suggested fix`）

### 変更後
- ファイル本文はタイトル・作成日のみ。メタ情報は `_index.yaml` のみに記録
- セクション構成:
  - `## 問題`（必須）: 何が問題かの説明
  - `## 水平展開`（任意）: 同じ問題がコードベース他所にも潜在しないか（issue-scan が記録）
  - `## 修正案`（任意）: 改善の方向性
  - `## ユーザーの言葉`（任意）: issue-create フロー用
- issue-scan スキル: `*-kit` 固有の記述を ref-inject ベースに変更。スキャン観点に保守性・水平展開を追加
...
Step 3. 各観点に ID ブロック割当（START_i = L+1+i*SLOT, SLOT=30）→ work:issue-scanner を N 個並列起動
        各サブエージェント: 観点をスキャン → {WT_PATH}/.work/issues/ に ISSUE-{N}.md を作成 → メタデータのみ返す
Step 4. 戻り値（メタデータのみ）を集約し {WT_PATH}/.work/issues/ の _index.yaml / _index.archive.yaml を更新
