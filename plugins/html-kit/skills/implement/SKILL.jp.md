...
# SKILL.jp.md — html-kit:implement(日本語ミラー)
...
**スキル名**: html-kit:implement
**トリガー**: UI 画面の実装 — モック(`html-kit:mock` 出力)や機能要件を実コードに落とす。
...
- このスキルはモックが既にあるか機能仕様が明確な前提。どちらもないなら先に `/html-kit:mock` を実行
  してフィードバックを集める。
