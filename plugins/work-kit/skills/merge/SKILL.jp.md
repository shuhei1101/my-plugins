...
1. マージ完了をユーザーに報告する
2. マージした PR の `TODO.md` を読み、`## 次PR候補` セクション（カラム: タイトル / 概要 / 実施条件）の内容を確認する
3. **次PR候補がある場合**: `/work-kit:pr-handoff` を自動で呼び出す（ユーザー確認不要）。pr-handoff は各候補を `実施条件` カラムで分類し、以下のように処理する:
   - **即時予約対象**（実施条件が空欄 / `-` / `即時実施可`、または既マージ済み PR への依存のみ）のみを実際に予約する
   - **依存後続候補**（同じテーブル内の他候補に依存するもの。例: `「{他候補}」が完了したら`）は即時予約せず、予約された先行 PR の `## 次PR候補` に転記され、連鎖的に引き継がれる
4. **次PR候補が空の場合**（プレースホルダー `{次にやること}` のままなど）: pr-handoff はスキップ
...
以下のいずれかであれば「候補なし」として pr-handoff をスキップする:
- `## 次PR候補` セクション自体がない
- テーブルのデータ行がプレースホルダー（`{次にやること}` など `{...}` を含む）のみ
- テーブルにヘッダー行・区切り行しかなくデータ行が存在しない
...
- [ ] 即時予約対象の次PR候補があれば pr-handoff で予約済み
- [ ] 依存後続候補があれば予約された先行 PR の `## 次PR候補` に転記済み
