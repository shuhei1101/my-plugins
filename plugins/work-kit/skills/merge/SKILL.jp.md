...
1. `docs/tasks/` 配下のフォルダを走査して PR フォルダ（`PR{N}/`）を列挙する
...
1. `docs/tasks/{date}_{title}/PR{N}/TODO.md` の `## TODO` セクションを読む
2. 未完了（`- [ ]`）のタスクがないか確認する
...
1. `docs/QA.md` を確認し、決定済みの事項を「解決済み」セクションへ移す
2. 変更があればコミットする:
...
1. マージ完了をユーザーに報告する
2. `docs/tasks/` に残っている進行中 PR があれば提示する
