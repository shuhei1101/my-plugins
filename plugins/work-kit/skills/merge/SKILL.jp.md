...
1. 今マージした PR の `TODO.md` の `## 次PR候補` テーブルを再読込する
   （ステップ11で使用したのと同じファイル）
...
3. 各候補行についてカテゴリを判定する:
   ...
   **a. 条件あり (Has conditions)**:
   - 3カラム目（実施条件）が他の候補を参照している — 例: `「{他候補}」が完了したら`
   - pr-handoff がこれらのブランチを意図的に予約しなかった
   - テーブルから直接リストアップする（ブランチ検索不要）

   **b. 着手可能 (Ready to start) または 他セッション進行中 (In progress elsewhere)**:
...
データソースは必ずマージした PR の `## 次PR候補` テーブルに限定する。
