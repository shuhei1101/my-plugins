...
対象 PR のドキュメントの `## QA` セクションを読み込み、未解決の各項目を `AskUserQuestion` ツールで最大 4 件ずつまとめて提示する。全質問への回答が完了した後、一括で PR ドキュメントに決定内容を反映する。
...
1. 現在の会話セッションで作業中の PR がある場合、その PR ドキュメントを最優先で使用する
2. 引数に PR 番号が明示されていれば（例: `PR139` または `139`）、それを使って PR ドキュメントを探す
3. 上記どちらも該当しない場合:
   - PR ドキュメントを検索: `find .work/tasks -type f -name "PR*.md"`
   - 1件のみ見つかった場合は自動選択
...
4. git ワークツリーも確認する:
   ...
   git worktree list
   ```
5. PR ドキュメントのパスを確定する（パターン: `.work/tasks/{YYMMDD}_{title}/{branch-hyphenated}.md`）
...
1. PR ドキュメントを読み込み、`## QA` セクションを特定する
...
### ステップ 4: PR ドキュメントの `## QA` セクションに全回答を一括反映する
...
1. ステップ 3 で収集した全回答をもとに、一括で PR ドキュメントの `## QA` セクションを更新する:
   - 「解決済み」: `**状態**: 解決済み — {決定内容または「その他」の自由入力}`
   - 「却下」: `**状態**: 却下 — {理由または「その他」の自由入力}`
   - 「保留」: 該当行をそのまま残す（変更しない）
2. 更新した PR ドキュメントを保存する
