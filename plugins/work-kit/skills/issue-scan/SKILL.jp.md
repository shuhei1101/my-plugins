...
### ステップ3: ファイルを Read して reference を取得する
...
3. 注入がなければ「このファイルに該当する `*-kit` reference は無い」と扱い、
   ステップ6へ進む（スキャン記録だけ書く）
...
1. 注入された各 reference と選ばれたファイルを照合し、以下を探す:
   - 規約違反（命名・型・コメント・スタイル）
   - アーキテクチャ違反（依存方向・レイヤー境界）
   - 改善余地（DRY 違反・冗長コード・古い書き方など、reference に記載された規約に基づくもの）
2. `wontfix` で既にクローズされているイシューと同じ内容は除外する
3. 独立して対処可能な単位ごとに整理する
