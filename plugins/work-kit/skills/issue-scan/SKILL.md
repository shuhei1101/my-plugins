...
---
<!-- Generated by work-kit/issue-scan v2.32.1 -->
...
and records newly discovered problems as issues in `.work/issues/`.
...
1. Read current `_index.yaml` (or start fresh if missing):
   ...
   issues: []
   ```
2. For each newly found problem:
   a. Increment `last_id`
   ...
   c. Create `.work/issues/ISSUE-{N}.md` with this structure:
      ...
      **Scan scope**: {scope}
      ...
      {File path and relevant line numbers or function names}

      ## Suggested fix

      {Concrete suggestion for how to address this}
      ```
...
4. Append a scan record to `_index.archive.yaml`:
   ```yaml
   scan_records:
     - date: {YYYY-MM-DD}
       skill: issue-scan
