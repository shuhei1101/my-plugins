...
name: pr-show
description: |
  Present next PR candidates in 3 categories (ready to start / in progress elsewhere / has conditions).
  Trigger when the user says "PR の状況を見せて", "着手可能なPRを一覧で", "予約済みPRを確認したい",
  "pr-show して", or invoked explicitly as `/work-kit:pr-show`.
  Also invoked by the merge skill at Step 12 — pass the merged PR's TODO.md path as the data source.
---
