<!-- Generated by claude-kit/skill-creator v3.17.1 -->
<!-- This file is a Japanese mirror. When updating the English original, update this file too. -->
# SKILL.jp.md — work-kit:work-start スキル（日本語ミラー）

> このファイルは `SKILL.md` の日本語ミラーです。Claude Code には読み込まれません。
> 変更する場合は JP ミラーを先に更新し、その後 `SKILL.md` にも反映してください。

---

**スキル名**: work-start
**トリガー**: ユーザーが「新しい PR を作って」「新しい作業を始めたい」「work-start して」と言ったとき

---

## 概要

新規 PR 作業を開始するスキル。
**ワークツリーを先に作成し**、その後ドキュメント類をワークツリー内に作成する。
（メインリポジトリに作業用ドキュメントが混入するのを防ぐ）

---

## 作業内容

### ステップ1: 次の PR 番号を決定する

#### 条件

- 常に — 最初に実行する

#### 処理内容

1. ユーザーが既に PR 番号・ブランチ名を指定している場合はその値を使う
2. 指定がない場合は以下を実行し、出力された番号を使う:

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/index-tool.py next-id .work/tasks/index.yaml
```

→ ステップ2へ進む

#### 出力

- 次の PR 番号が確定している

---

### ステップ2: 依頼内容を整理する

#### 条件

- ステップ1が完了していること

#### 処理内容

1. 以下を決定する:
   - **タイトル**: タスクフォルダ名に使う短い説明（kebab-case）
   - **タイプ**: `feat` / `fix` / `refactor` / `docs` / `chore` / `test`
   - **TODO リスト**: 今回やることのチェックリスト
   - **関連ノート**: 対応するノートが `.work/notes/` にあるか、新規作成か
   - **不明点**: 決まっていないことがあれば

→ ステップ3へ進む

#### 出力

- タイトル・タイプ・TODO・ノート情報・不明点が確定している

---

### ステップ3: index.yaml にエントリを追加する（メインリポジトリ）

#### 条件

- ステップ2が完了していること

#### 処理内容

1. 以下を実行して PR エントリを追加する:

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/index-tool.py add .work/tasks/index.yaml \
  --id {N} \
  --title "PR{N} — {title}" \
  --type {type} \
  --summary "{summary}" \
  --task "{YYYYMMDD}_{title}"
```

→ ステップ4へ進む

#### 出力

- `.work/tasks/index.yaml` に PR エントリと `last_id` が更新済み（メインリポジトリ）

#### 注記

- `index.yaml` は `.work/tasks/.gitignore` で除外されているため、マスターへのコミットは不要

---

### ステップ4: ワークツリーとブランチを作成する（有効時のみ）

#### 条件

- ステップ3が完了していること

#### 処理内容

1. ワークツリー利用が有効かを確認する。**デフォルトで有効**であり、環境変数
   `WORK_KIT_USE_WORKTREE` を falsy 値（`false` / `0` / `no` / `off`）に設定したときのみ無効になる:

   ```bash
   v="${WORK_KIT_USE_WORKTREE:-true}"; case "${v,,}" in false|0|no|off) echo disabled;; *) echo enabled;; esac
   ```

2. **有効の場合**: `/work-kit:work-add` を PR 番号・ブランチ付きで呼び出す:

   > `/work-kit:work-add PR{N} {type}/{title}`

3. **無効の場合**: ワークツリー作成をスキップし、ユーザーに通知する:

   > ⚠️ `WORK_KIT_USE_WORKTREE` が無効のため、ワークツリーの作成をスキップします。  
   > `.work/` フォルダ管理のみで作業を続けます。  
   > ワークツリーを使用したい場合は `settings.json` の `env` から `WORK_KIT_USE_WORKTREE` を外すか `true` に設定してください。

→ ステップ5へ進む

#### 出力

- (worktree 有効) ワークツリーが `../repo-wt-PR{N}` に作成済み、ブランチ `PR{N}/{type}/{title}` が存在する
- (worktree 無効) ワークツリーなし。`.work/` フォルダ管理のみで継続する

#### 補足

##### 禁止事項

- メインブランチ（master/main）に直接コミットしない

---

### ステップ5: タスクフォルダを決定する（AI 自律判断）

#### 条件

- ステップ4が完了していること

#### 処理内容

1. ワークツリー内の `.work/tasks/` 配下のフォルダ名をすべて確認する
2. 各フォルダ名（`YYYYMMDD_title` 形式）と今回の PR の内容を照合し、以下の基準で判断する:
   - **既存フォルダに追加する**: 同じ大きな目的・機能領域のタスクが既存フォルダとして存在し、今回の PR がその一部として自然にまとめられる場合
     - 例: 同じ機能を複数 PR に分割している、バグ修正の続き、関連するリファクタリング
   - **新規フォルダを作成する**: 既存フォルダとの関連が薄い、または `.work/tasks/` が空の場合

3. 判断結果に応じて以下を確定させる:
   - 既存フォルダに追加 → `--task-dir {folder_name}` を使用
   - 新規作成 → `--date {YYYYMMDD} --title {title}` を使用

→ ステップ6へ進む

#### 出力

- タスクフォルダの種別（新規 or 既存）と使用する引数が確定している

#### 補足

- ユーザーへの確認は不要。AI が内容を見て自律的に判断する
- 判断に迷う場合は新規作成を選ぶ（後からまとめることはできる）

---

### ステップ6: タスク・PR フォルダと TODO.md・QA.md を作成する（ワークツリー内）

#### 条件

- ステップ5が完了していること

#### 処理内容

ステップ5の判定結果に応じて以下のいずれかを実行する:

**新規タスクフォルダの場合:**

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/setup-task.py \
  ../$(basename $(pwd))-wt-PR{N} \
  --pr {N} \
  --title {title} \
  --date {YYYYMMDD} \
  --plugin-root ${CLAUDE_PLUGIN_ROOT}
```

**既存タスクフォルダに追加する場合:**

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/setup-task.py \
  ../$(basename $(pwd))-wt-PR{N} \
  --pr {N} \
  --task-dir {existing_folder_name} \
  --title {title} \
  --plugin-root ${CLAUDE_PLUGIN_ROOT}
```

→ ステップ7へ進む

#### 出力

- `../repo-wt-PR{N}/.work/tasks/{task_folder}/PR{N}/TODO.md` 作成済み
- `../repo-wt-PR{N}/.work/tasks/{task_folder}/PR{N}/QA.md` 作成済み

---

### ステップ7: TODO.md に作業内容を記載する（ワークツリー内）

#### 条件

- ステップ6が完了していること

#### 処理内容

ワークツリー内の `TODO.md` を開き、テンプレートのひな形行を実際の作業内容に書き換える。
以下の項目は必ず含める（削除・省略禁止）:

| 完了 | 作業内容 |
|---|---|
| - | QA.md に未決定事項を記録する |
| - | `.work/notes/` のノートを更新する |
| - | （実装タスク: PR 固有の作業内容に書き換える） |
| - | ルール・CLAUDE.md を整備する |

加えて、`## 概要` と `## 作業内容` の間のサブセクションを埋める:

**`### 実施条件`**:
- このPRをいつ着手できるかを書く: `即時実施可` または `「{他PR名}」が完了してから`
- 起動元PRの `## 次PR候補` テーブルの `実施条件` カラムの内容と対応する

**`### 関連PR`**:
- このPRと直接関連するPR（前提PR・分割兄弟PR・後続PRなど）を列挙する
- 表形式: `| PR番号 | 概要 |` — 関連PR 1件ごとに1行
- 関連PRがなければプレースホルダー行のまま残す

加えて、`## 関連イシュー` セクションを埋める（`## 参考ドキュメント` と `## 次PR候補` の間）:
- このPRが解決する `.work/issues/ISSUE-{N}` のイシューを列挙する
- 表形式: `| ID | 概要 | resolution |` — イシュー1件ごとに1行
- `resolution` は `resolved`（このPRで修正する）または `wontfix`（意図的に修正しない）
- これらの行は `merge` スキルの自動クローズ処理の対象（イシューファイルが `.work/issues/closed/` に移動され、`_index.archive.yaml.closed_issues` に追記される）
- **関連イシューがない場合**: `## 関連イシュー` の見出しと表ごと丸ごと削除する（空のプレースホルダーは残さない）

加えて、TODO.md 末尾の `## 次PR候補` セクションを埋める（カラム: タイトル / 概要 / 実施条件）:
- 今回のセッションで「次にやること」や将来のPRが言及されていればここに列挙する
- 各候補の `実施条件` を埋める:
  - 依存なし → `即時実施可`（または `-`）
  - 同じテーブル内の他候補に依存（直列依存）→ `「{他候補タイトル}」が完了したら`
- 言及がなければプレースホルダー行のまま残す（セクション自体は削除しない）

→ ステップ8へ進む

---

### ステップ8: ノートを整備する（ワークツリー内）

#### 条件

- ステップ7が完了していること

#### 処理内容

1. ワークツリー内の `.work/notes/` を確認する
2. 関連するノートが存在する場合 → 今回の変更に関わる箇所を更新する
3. 存在しない場合 → テンプレート（`${CLAUDE_PLUGIN_ROOT}/templates/note.md` 参照）を元に新規作成する
4. TODO.md の「参考ドキュメント」セクションにノートへのリンクを追記する

→ ステップ9へ進む

---

### ステップ9: 不明点を QA.md に記録する（ワークツリー内）

#### 条件

- ステップ8が完了していること

#### 処理内容

1. ステップ2で確認した不明点をワークツリー内の `PR{N}/QA.md` に QA-XXX として追記する
2. 不明点がない場合はスキップする

→ ステップ10へ進む

---

### ステップ10: 作成内容をコミットしてユーザーに報告し、実装を開始する

#### 処理内容

1. ワークツリー内の作成ファイルをすべてコミットする（ブランチ: `PR{N}/{type}/{title}`）
2. 作成した内容を報告する:
   - ブランチ名・ワークツリーパス・TODO.md パス・ノートパス
3. 実装を開始する:
   - **QA がある場合** → ユーザーに確認を求めてから実装を開始する
   - **QA がない場合** → そのまま実装を開始する

#### 補足

##### 禁止事項

- 作成したワークツリー（`PR{N}/{type}/{title}` ブランチ）以外へのコミット禁止

##### コミット粒度

- コミットはユーザーが見てわかりやすい意味のある単位で切る
- 細かく切りすぎない
- 実装計画（TODO・仕様書など）と実装コードを同じコミットに混在させない

##### コミットメッセージの言語

- このスキルから生成するすべてのコミットメッセージは**日本語**で記述する
- 件名・本文ともに日本語で書く（`Co-Authored-By:` などのメタデータ行は英語のままで可）
- `feat:` `fix:` `chore:` などの conventional commit プレフィックスは英語のまま使用してよい
- 例: `chore: PR{N} のタスクドキュメントを作成 #PR{N}`


---

## 必須: 生成物に出自メタデータを付与する

このスキルが書き出したファイルには、必ず出自メタデータを付与すること。
これにより `claude-kit:version-sync` が古いプラグインバージョンで生成された
ファイルを検出できるようになる。

**手順**（このスキルが作成・上書きする全ファイルに対して実行）:

1. スタンプ用スキルを起動し、このスキルの識別情報と対象ファイルを渡す:

   ```
   /claude-kit:mark-generated --plugin work-kit --skill work-start --file {generated-file-path}
   ```

2. 返ってきたコメント文字列を、ファイルに YAML frontmatter がある場合は **その閉じ `---` の直後** に、frontmatter がない場合はファイル最上部に挿入する。frontmatter の前（`---` より上）には絶対に置かない。Markdown レンダラが frontmatter として認識しなくなる。
3. ファイル先頭に既に `<!-- Generated by` で始まる行（または `*.py` / `*.sh` / `*.yaml` の場合は `# Generated by`）が存在する場合は、その1行を置き換える（重複させない）。

JP ミラーファイル（`*.jp.md`）の場合は、メタデータ行と JP ミラー警告行の2行が
返ってくるので、両方をそのまま貼ること。

`mark-generated` が空文字列を返した場合（例: `*.json` はコメント不可）は、
何もせずに次へ進む。
