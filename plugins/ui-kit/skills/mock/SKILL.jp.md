# SKILL.jp.md — ui-kit:mock(日本語ミラー)
...
1. 全文読み込み:
   ...
   ```
   {plugin_root}/references/principles.md   # FLOCSS、デザイントークン、JS 規約
   {plugin_root}/references/ui-design.md    # 画面タイプ別 UX パターン
   ```

2. **プロジェクトの共通リソースを棚卸しする**(モックでも必須 — 各案が並行に別物を作らないため):
   - `static/js/constants.js`(相当) — デザイントークン、ブレイクポイント
...
- **モバイル対応**:`ui-design.md` のブレイクポイント(640 / 1024)でメディアクエリ
...
- `{plugin_root}/references/principles.md` — FLOCSS、デザイントークン、JS 規約
- `{plugin_root}/references/ui-design.md` — 画面タイプ別 UX パターン、レスポンシブ
