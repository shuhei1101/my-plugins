...
モーダルヘッダの 🎯「要素選択」ボタンで要素ピッカーモードに入る。ホバーで要素がハイライトされ、
クリックで JSON(URL + XPath + tag + classes + 短縮テキスト)がクリップボードへコピーされる。
XPath 形式(short / full)はモーダルで設定可能。

「この特定の要素を直して」と Claude に伝えたいときに、依頼文と合わせて貼り付けると便利。
