...
| **2 ペイン(一覧 + 詳細)**       | 一覧を見ながら詳細を見る(PC)。モバイルは一覧 → 詳細遷移にフォールバック |
...
- 高さ: PC 56–64px、モバイル 48–56px

### サイドバー(PC)

- 左固定、フル高
- PC(≥ 1024px)では常時表示、モバイルはドロワーへ折りたたみ
- 含める項目:
...
Foundation で CSS Custom Properties またはメディアクエリエイリアスとして定義する。

### 区分別の振る舞い

| パターン              | PC                              | モバイル |
|---|---|---|
| サイドバー            | 常時表示(固定)                | ドロワー(ハンバーガー) |
| 2 ペイン 一覧 + 詳細  | 横並び                          | 単一カラム、一覧 → 詳細遷移 |
| 3 ペイン              | 3 カラム                        | 単一カラム、タブまたは戻るナビ |
| トップタブ            | 横並び                          | はみ出したら横スクロール |
| アクションボタンバー  | スティッキーフッター or 右上    | 画面下部固定(全幅) |
| タッチターゲット      | 32px 以上                       | **44px 以上**(Apple HIG) |
...
- 左ペイン(PC 幅 280–360px): 項目の一覧
  ...
  - 選択中項目: ハイライト
- 右ペイン: 選択項目の詳細
- モバイル: 一覧を表示、項目タップで詳細へ遷移、戻るボタンで一覧へ
...
- ローディングスケルトン・スピナー・プログレスバー
- 空状態カード・エラーカード・トースト通知
- ラベル / ヘルプ / エラーレイアウトを持つフォームフィールド
- 確認ダイアログ(モーダル)
- キーボードショートカット・ヘルプモーダル
- ドロワー・サイドバー・ヘッダー
- タブバー・カードグリッド・アクションバー
- テーマトグル、モーション対応ラッパー

それぞれはプロジェクトの `c-*`(Component)レイヤーに置く。ページはクラス名と JS インポートで
再利用するのみ。自前でトースト・ダイアログ・スケルトンを再実装しない。

画面の実装・モック作成時は **必ず最初に既存の共通コンポーネント**(`c-*`、`p-*`)を確認する。
強制ワークフローは `ui-kit:implement` スキルを参照。
