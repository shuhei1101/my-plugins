...
flowchart TD
  ...
  NAR -->|/gh-kit:pr-review-auto| Reviewed[マージ可能 ラベル付与]
...
| 4 | `/gh-kit:pr-draft-create-auto` | `確認:pr-planner` 付き Issue 全件 → Draft PR を作成 |
...
| 8 | `/gh-kit:pr-review-auto` | `needs-ai-review` Ready PR を直列でレビュー → 合格 + assignees 未設定なら `マージ可能` ラベル付与 |
| 9 | `/gh-kit:pr-merge` | `マージ可能` ラベル付き PR を 1 件 base へマージし worktree 削除・push まで実行 |
| 10 | `/gh-kit:pr-merger-auto` | `マージ可能` ラベル付き Ready PR を Monitor 方式で直列マージ |
...
| 5 | `pr-reviewer` | `/gh-kit:pr-review-auto` | レビュー → 合格時は `マージ可能` ラベルを付与して `pr-merger` へ委譲 |
| 6 | `pr-merger` | `/gh-kit:pr-merger-auto` | `マージ可能` PR を base 取り込み・マージ・`worktree_remove`・push まで実行 |
...
| `確認:pr-planner` | AI レビュー完了・PR 作成 OK（`issue-review` が `needs_user_review: false` 判定時に付与。`pr-draft-create-auto` の起動契機） |

### gh-kit フロー制御（processing 細分）

| ラベル | 意味 |
|---|---|
| `処理中:pr-planner` | Draft PR 作成処理中（`pr-draft-create-auto` が付与） |
| `処理中:pr-implementer` | 実装エージェントが実装中（`pr-implement-auto` が付与） |
| `処理中:pr-reviewer` | レビューエージェントがレビュー中（`pr-review-auto` が付与） |
...
|---|---|
| `wip` | Draft 雛形 PR |
| `マージ可能` | AI レビュー OK でマージ可（`pr-review` が付与、`pr-merger` がマージ後に除去） |
