...
| 4 | `pr-implementer` | `/gh-kit:pr-implement-auto` | 既存 Draft PR に実装コミットを積み Ready 化、`gh-kit:needs-user-review` 要否を返す |
...
| `gh-kit:needs-ai-review` | AI レビュー必要（必ず付く） |
| `gh-kit:needs-user-review` | ユーザーレビュー必要（AI 判定で付く） |
