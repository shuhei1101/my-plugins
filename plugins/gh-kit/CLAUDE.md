...
詳細は Wiki: **`gh-kit/テンプレート/イシュー本文.md`**

担当モニター・サブセクション一覧・記入テンプレートはすべて Wiki に集約。

> Issue は「**何を作るか**」（要件レベル）まで。UI 設計・システム方式設計（SS）・PoC 結果・実装計画・テスト計画・ドキュメント変更計画は **PR 本文側** に移管（PR diff でレビューできるようにするため）。
...
| --------------------------- | -------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `## 紐づく Issue`           | -                                | 必須                               | 親 Issue 番号                                                                                  | issue-spec（Draft PR 作成時に記入）                  |
| `## システム方式設計（SS）` | `### 採用ライブラリ`             | 実装系                             | No / ライブラリ / バージョン / 用途 / 補足 の表                                                | pr-arch                                              |
| 〃                          | `### コンポーネント分割`         | 〃                                 | No / 新規/変更 / レイヤー / コンポーネント / 役割 / 補足 の表                                  | 〃                                                   |
| 〃                          | `### データフロー`               | 複数コンポーネント間でデータが流れる場合 | Mermaid sequenceDiagram または flowchart で図示                                            | 〃                                                   |
| `## UI 設計`                | `### 画面構成`                   | 画面ありの場合                     | 画面ごとに小見出し + 要素/種類/位置/データソース/アクション の表                               | pr-ui                                                |
| 〃                          | `### 画面遷移`                   | 画面ありで複数画面が絡む場合       | Mermaid flowchart LR                                                                           | 〃                                                   |
| 〃                          | `### モック`                     | 画面ありの場合                     | No / 画面 / URL / 補足 の表                                                                    | 〃                                                   |
| `## PoC 結果`               | `### 検証したライブラリ`         | ライブラリ選定で PoC を実施した場合 | No / ライブラリ / バージョン / ライセンス / 用途 / 補足 の表                                   | pr-arch または pr-ui                                 |
| 〃                          | `### 動作確認結果`               | 〃                                 | No / ライブラリ / 検証項目 / 結果 / 補足 の表 + 最小再現コード                                 | 〃                                                   |
| 〃                          | `### 追記した Wiki`              | Wiki 追記/作成あり時               | No / ページ / 内容 / 補足 の表（reset 用の履歴）                                               | 〃                                                   |
| `## 実装計画`               | -                                | 必須                               | No / 完了 / 新規/変更 / レイヤー / 分類 / ファイル / 対象 / 概要 / 補足 の表（分類: クラス/メソッド/関数/コンポーネント/フック/型/DBカラム/マイグレーション/エンドポイント など。対象: クラス名・メソッド名・型名・カラム名など。概要: 何を追加/変更するかを 1〜2 文。完了は ⬜/✅、pr-impl が完了から ✅ に更新） | pr-arch が SS 設計と同時に記入 / pr-ui が UI 関連行追加 / pr-impl がチェック |
| `## テスト計画`             | `### 単体テスト`                 | 実装系（ドキュメント系はスキップ） | 新規/変更/既存実行のテスト一覧 + 各行にチェックボックス（pr-impl で Green になったらチェック） | pr-test が追加 / pr-impl がチェック |
| 〃                          | `### 結合テスト`                 | 〃                                 | 〃                                                                                             | 〃                                                   |
| 〃                          | `### E2Eテスト`                  | 〃                                 | 〃                                                                                             | 〃                                                   |
...
| 〃               | 外部ライブラリ/{lib名}.md                | このプロジェクトで使うメソッドとそのパラメータを公式ドキュメントから抜粋。書き方規約は `gh-kit/テンプレート/外部ライブラリ.md` |
| 外部API          | 外部API/README.md                        | 採用済み外部 API のインデックス                              |
| 〃               | 外部API/{API名}.md                       | このプロジェクトで使うエンドポイントを公式ドキュメントから抜粋。書き方規約は `gh-kit/テンプレート/外部API.md` |
| テスト           | テスト/テスト実行方法.md                 | **プロジェクト固有**のテスト起動コマンドと前提条件（戦略自体は rules 側） |
| 〃               | テスト/テスト一覧.md                     | 既存テスト（種別 × 対象モジュール）の一覧と網羅状況          |
...
**廃止 / 統合済ページ:**
- `モジュール構成_{モジュール名}.md` → `設計図/モジュール構成/{分類}.md` に階層化
- `シーケンス図_{機能名}.md` / `状態遷移図` / `画面遷移図` / `データフロー図` / `フローチャート` → `設計図/バックエンド結合/` `設計図/フロントエンド結合/` `設計図/シナリオ/` に統合
- `ER図.md`（単一）→ `設計図/ER図/` 配下に分類分割（DB だけでなく JSON / YAML も対象）
- `APIエンドポイント一覧.md` → `設計図/バックエンド結合/README.md` に集約
...
`pr-arch` の領域別検討は**カスタムサブエージェントで並列処理**する。
...
- **issue-spec が後で埋めるセクション骨組み**（要件レベル: 機能要件 / 非機能要件 / スコープ外）を本文に作っておく
...
2. 後続フェーズ（issue-spec）のセクション骨組みを本文に作成
3. 内容を表すタイトルに更新
4. `type:*` ラベル・優先度ラベルを付与
5. **現状調査（全てサブエージェント並列起動）**:
   - 5a. **領域別コードベース調査**: Issue が言及する領域ごとにサブエージェントを起動（関連カスタムサブエージェントがあれば優先、なければ汎用 Agent でOK）→ コードベース・既存実装・関連ファイルの所在と要点を返してもらう。各サブエージェントには関連 Wiki（該当領域の `設計図/モジュール構成/{分類}.md` / `設計図/バックエンド結合/{論理名}.md` / `設計図/フロントエンド結合/{論理名}.md` / `設計図/シナリオ/{論理名}.md`（あれば）/ `設計図/ER図/{分類}.md` など）を**注入**し、既存のデータ構造との関連も把握させる
...
| `### 関連実装コード`     | Issue が言及する領域のコードベース調査                | **No / ファイル / メソッド / 概要 / 補足** の表                                              | 設計図/モジュール構成/*.md / 設計図/バックエンド結合/*.md / 設計図/フロントエンド結合/*.md / 設計図/シナリオ/*.md |
...
- 「こうあるべき」「こうした方がいい」は spec / arch / pr-arch で決めるので、ここでは書かない
...
システム要件（SA）を確定する。Issue フェーズの最終ステップで、完了後はユーザー手動で `確認:pr-ui`（画面あり）/ `確認:pr-arch`（画面なし）を付与して PR フェーズに進む。

- 機能要件・非機能要件・スコープ外を Issue 本文に整理（エラーハンドリング・バリデーションは機能要件のカテゴリに含める）
- 要件で曖昧な点があればユーザーに質問するコメントを投稿（1質問 = 1コメント）
- 完了後は `確認:issue-spec` + `フェーズ終了` を除去し、次ラベルは付けない（ユーザーが要件 OK を確認してから手動付与: **画面ありなら `確認:pr-ui` / 画面なしなら `確認:pr-arch`**）
...
| サブセクション   | 入力値                                | 概要                                                                                                                                                       | 参照 Wiki                                                                                                                                                                                              |
| ---------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `### 機能要件`   | 概要・背景・現状・ユーザー対話        | **No / カテゴリ / 要件 / 補足** の表。カテゴリは 編集機能/閲覧機能/バリデーション/エラー表示/状態表示/**レスポンシブ対応** など。エラーハンドリング・レスポンシブ対応の有無もここに含める | 該当領域の `設計図/バックエンド結合/README.md` + `{論理名}.md` / `設計図/フロントエンド結合/README.md` + `{論理名}.md`                                                                                 |
| `### 非機能要件` | ユーザー要望・既存システムの SLA      | **No / カテゴリ / 要件 / 補足** の表。カテゴリは 性能/セキュリティ/運用 など。当てはまるものがある時のみ書く                                               | 該当領域の `設計図/モジュール構成/{分類}.md`（セキュリティ関連） / `設計図/ER図/{分類}.md`（DB コネクション・インデックス等） / `外部API/{API名}.md`（レートリミット・料金） / `外部ライブラリ/{lib名}.md`  |
| `### スコープ外` | 機能要件・関連 Issue・ユーザー対話    | **No / 項目 / 理由 / 補足** の表。「今回はやらないこと」を明示。**別 Issue に切り出す判断は必ずユーザー確認が必要**（勝手に起票 / リンク追記しない）        | -                                                                                                                                                                                                      |
...
| エージェント                               | 入力                                                             | 出力                                                                                                             | 注入する Wiki                                                                                                                                                                                                                      |
| ------------------------------------------ | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| gh-kit:functional-requirements-analyzer    | Issue 本文（概要・背景・現状） + Issue 番号 + 対象領域           | 機能要件観点リスト（重複エンドポイント / 影響画面 / エラーハンドリング候補 / バリデーション候補 / 状態表示候補） | 該当領域の `設計図/バックエンド結合/README.md` + `{論理名}.md` / `設計図/フロントエンド結合/README.md` + `{論理名}.md` / `設計図/シナリオ/README.md`                                                                               |
| gh-kit:nonfunctional-requirements-analyzer | Issue 本文（概要・背景・現状） + Issue 番号 + 採用予定の外部依存 | 非機能要件観点リスト（性能上限 / セキュリティ制約 / 運用上の注意 / インフラ制約 / 外部 API 制約）                | 該当領域の `設計図/モジュール構成/{分類}.md`（セキュリティ関連分類） / `設計図/ER図/{分類}.md`（DB 接続数・インデックス・履歴等） / `外部API/{API名}.md` / `外部ライブラリ/{lib名}.md` / 新規外部 API 利用時は公式ドキュメント URL |

- ステップ 2a（機能要件）/ 2b（非機能要件）冒頭で **並列起動** して観点を洗い出し、その結果を踏まえて表を書く
- 出力は **観点リスト**（要件そのものではない）。issue-spec が観点を要件文にまとめて表に落とし込む
- 新規外部 API / 新規ライブラリの採用検討時は該当エージェントに公式ドキュメント URL も渡し、破壊的変更・レートリミット等を確認させる

**フロー**:

1. **要件観点調査（サブエージェント並列起動）**:
   - `gh-kit:functional-requirements-analyzer` と `gh-kit:nonfunctional-requirements-analyzer` を **並列起動**
   - 各エージェントに Issue 本文 + 対象領域 + 該当 Wiki ページを注入し、観点リストを返してもらう
   - 観点を踏まえて `## システム要件（SA）` の 3 サブセクションに落とし込む（`### 機能要件` / `### 非機能要件` / `### スコープ外`）
   - スコープ外は「今回はやらないこと」を明示するのみ。**別 Issue 化はユーザー確認必須**（勝手に起票しない）
2. 曖昧な点があれば 1質問1コメントで投稿し、ユーザー回答を本文に反映
3. 完了報告コメント投稿 + `assignee=ユーザー` で待機
4. **ユーザー応答ループ**:
   - フィードバックコメント + assignee 外す → 反映して 3 に戻る
   - `フェーズ終了` ラベル付与 → 次へ
5. **Draft PR 作成**（要件確定後の引き継ぎ準備）:
   - `worktree_create` MCP で worktree 作成（ブランチ名: `feat/issue-{N}-{slug}` 等）
   - PR 本文テンプレートに沿った空コミットを push
   - `gh pr create --draft` で Draft PR を作成
   - PR 本文 `## 紐づく Issue` に親 Issue 番号を記入
   - PR 本文に後続フェーズ（pr-ui / pr-arch / pr-test / pr-impl / pr-doc-plan / pr-doc）のセクション骨組みを配置（ヘッダーのみ、内容は後続が埋める）
6. **ラベル更新**（前提条件: 自分宛コメントが全て本文反映済み（未反映あればユーザー確認後に自分宛のみ一括 Resolve）+ Draft PR 作成済み）:
   - 上記条件を満たさなければ「本文反映 → 一括 Resolve → Draft PR 作成」を先に実行
   - 満たしたら `確認:issue-spec` 除去 + `フェーズ終了` 除去（次ラベルは付与しない — **ユーザーが要件 OK 確認後、PR に手動で `確認:pr-ui`（画面あり）/ `確認:pr-arch`（画面なし）を付与**）（フロー終了）

**ラベル更新**（フェーズ完了時=ユーザー `フェーズ終了` 付与後）:
- Issue: 除去 `確認:issue-spec` + `フェーズ終了` / 付与 なし
- PR: 新規作成（Draft）+ ラベル付与なし（ユーザーが手動で `確認:pr-ui` / `確認:pr-arch` を付与）


### 3. pr-ui（任意）

**モニター条件**:
- PR に `確認:pr-ui` ラベルが付与された
- Assignee にユーザが設定されていない

UI 設計を行う。**UI ライブラリ採用時に必要なら PoC まで実施（pr-arch の PoC 機能と同じ手順）**。

- 画面構成・画面遷移を提案
- 必要に応じてモック画面を作成
  - 一画面につき 1コメントで各案のモック URL を貼る
- UI ライブラリ採用検討で PoC が必要な場合は **pr-arch のフロー（PoC 要否判定カテゴリ A〜E / PoC worktree 運用ルール）を参照して同手順で実施** → 採用ライブラリを `## PoC 結果` に記録
- ユーザー確認後、SS 設計に進む
...
7. **ラベル更新**（前提条件: 自分宛コメントが全て本文反映済み（未反映あればユーザー確認後に自分宛のみ一括 Resolve））:
   - 上記条件を満たさなければ「本文反映 → 一括 Resolve」を先に実行
   - 満たしたら `確認:pr-ui` 除去 + `フェーズ終了` 除去 + `確認:pr-arch` を付与（フロー終了）

**ラベル更新**（フェーズ完了時=ユーザー `フェーズ終了` 付与後）:
- Issue: なし
- PR: 除去 `確認:pr-ui` + `フェーズ終了` / 付与 `確認:pr-arch`


### 4. pr-arch

**モニター条件**:
- PR に `確認:pr-arch` ラベルが付与された
- Assignee にユーザが設定されていない

システム方式設計（SS）+ 実装計画を行う。コンポーネント分割・採用ライブラリ・データフロー・実装計画（コード変更一覧）を決定する。**ライブラリ選定で必要な場合は PoC まで実施**する。
...
- SS 設計と同時に `## 実装計画` 表を埋める（pr-ui が前に走った場合は UI 関連行も含めて統合）
- ユーザーが各論点を確認した後、`確認:pr-test` を自動付与して次フェーズに進む
...
- `## システム方式設計（SS）`
  ...
  - `### コンポーネント分割`
  - `### データフロー`
- `## 実装計画`（SS 設計と同時に記入。pr-ui が前に書いた UI 関連行はそのまま残す）
...
| `## 実装計画`            | SS（コンポーネント分割・データフロー）・既存クラス・サブエージェント調査 | **No / 完了 / 新規/変更 / レイヤー / 分類 / ファイル / 対象 / 概要 / 補足** の表。分類は クラス/メソッド/関数/コンポーネント/フック/型/DBカラム/マイグレーション/エンドポイント など。概要は「何を受け取って何を返すか」を 1〜2 文。完了は ⬜（pr-impl が ✅ に更新）。pr-ui が前に書いた UI 関連行（コンポーネント等）はそのまま残す | 設計図/モジュール構成/*.md / 設計図/ER図/*.md |
...
4. **PoC 要否判定（ライブラリ系のみ対象）**: 採用候補が未経験で PoC が必要か判定（前述「PoC 要否判定カテゴリ」A〜E に該当するか）
   ...
   - **必要** → 以下の PoC サブステップを実施:
     ...
     - 4f. **Wiki 反映**: `外部ライブラリ一覧.md` に行追加・`外部ライブラリ_{lib名}.md` を新規作成または更新（書き方規約は Wiki `gh-kit/テンプレート/外部ライブラリ.md` 参照: 概要 / 現在のバージョン情報 / インストール / 使用するメソッドとパラメータ）→ 追記/新規作成した URL を `### 追記した Wiki` に記録
     - 4g. PoC worktree とローカルブランチを**全て削除**（大規模 PoC で pr-arch に引き継ぐ場合のみ採用案の worktree を保持し、本文に注記）
...
8. **ラベル更新**（前提条件: 自分宛コメントが全て本文反映済み（未反映あればユーザー確認後に自分宛のみ一括 Resolve）・PoC 実施時は非決定案の worktree が全て削除済み）:
   ...
   - 満たしたら PR の `確認:pr-arch` 除去 + `フェーズ終了` 除去 + `確認:pr-test` を付与（フロー終了）

**ラベル更新**（フェーズ完了時=ユーザー `フェーズ終了` 付与後）:
- Issue: なし
- PR: 除去 `確認:pr-arch` + `フェーズ終了` / 付与 `確認:pr-test`
...
**担当セクション（PR 本文）**:
- `## テスト計画`（pr-arch が骨組み済みの各サブセクションを更新）

**担当セクション詳細**:

| サブセクション                                            | 入力値                       | 概要                                                                                                       | 参照 Wiki                              |
| --------------------------------------------------------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| `## テスト計画`（pr-arch が骨組み済の各サブセクション）   | pr-arch で立てた計画         | 計画通りにテストコードを Red 状態で作成し、各チェックボックスの行にテストファイル名を確定記入              | テスト実行方法.md / テスト戦略.md      |
...
1. PR 本文 `## テスト計画`（pr-arch が作成済み）の各サブセクション（`### 単体テスト` / `### 結合テスト` / `### E2Eテスト` / `### 外部疎通テスト`）を読み込む
...
6. **ユーザー応答ループ**（テスト実装に対するフィードバック）:
   ...
   - 計画自体の見直しが必要な場合はラベルを `確認:pr-arch` に戻して終了
   - `フェーズ終了` ラベル付与 → 次へ
7. **ラベル更新**（前提条件: 自分宛コメントが全て本文反映済み（未反映あればユーザー確認後に自分宛のみ一括 Resolve））:
   - 上記条件を満たさなければ「本文反映 → 一括 Resolve」を先に実行
...
| サブセクション                              | 入力値                       | 概要                                                                                                          | 参照 Wiki |
| ------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------- | --------- |
| `## 実装計画`（チェックボックス更新）       | pr-arch の実装計画           | 完了したタスクから順にチェックを入れる（本文の追記はせず、既存項目のチェック更新のみ）                        | -         |
| `## テスト計画`（チェックボックス更新）     | pr-test の追加テスト         | Green になったテストから順にチェックを入れる                                                                  | -         |

**ユーザーとのコメントのやり取り**: なし（実装に専念。設計レベルで困った場合のみラベルを `確認:pr-arch` に戻して引き継ぎ）
...
6. **判定**:
   ...
   - **設計レベルの判断に迷う**（メソッドシグネチャ変更などが必要） → ラベルを `確認:pr-arch` に戻して終了（pr-arch に引き継ぎ）
   - **テスト失敗が解消できない** → ラベルを `確認:pr-arch` に戻して終了（テスト計画/実装計画の見直しを依頼）
...
1. **既存ドキュメント構造の調査（サブエージェント並列起動）**:
   ...
   - 各サブエージェントには **Issue 本文 + PR 本文** + 該当領域の関連 Wiki ページを注入
...
issue-triage が起票直後に全セクションの骨組みを作成し、後続モニター（issue-spec）が担当セクションだけ上書きする。担当一覧・テンプレート・記入例は Wiki 参照。

#### PR 本文テンプレート

pr-arch が起票直後に骨組みを作成し、後続モニターが自分の担当セクションだけ上書き更新する。
...
## 実装計画
<!-- pr-arch が計画・pr-impl が完了したものから「完了」列を ✅ に更新 -->
...
## テスト計画
<!-- pr-arch が骨組み・pr-test が中身を記入・pr-impl が「完了」列を ✅ に更新 -->
...
| -- | ---- | ----------------------------------- | ------------------ | ----------------------------------------- | ----------------------------- |
| 1  | ⬜   | `設計図/モジュール構成/ユーザー.md` | `### UserService`  | `updateProfile()` メソッド追記            | 要件由来                      |
| 2  | ⬜   | `設計図/バックエンド結合/ユーザー編集.md` | -            | 新規ファイル作成（PUT /api/users/{id}）   | 要件由来                      |
| 3  | ⬜   | `設計図/ER図/ユーザー.md`           | `### users`        | `bio` カラム追記                          | 要件由来                      |
| 4  | ⬜   | `外部ライブラリ/react-image-crop.md` | -                | 新規ページ作成                            | 採用ライブラリの使い方        |
| 5  | ⬜   | `外部ライブラリ/README.md`          | -                  | `react-image-crop.md` 行追加              | 要件由来                      |
