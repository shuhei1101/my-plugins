...
| --------------------------- | ------------------------ | ---------------------------------------- | -------------------------------------- | ------------ |
| `## 概要`                   | -                        | 必須                                     | Issue の目的（ユーザー入力範囲内）     | issue-triage |
| `## 背景`                   | -                        | 〃                                       | 起票に至った経緯（ユーザー入力範囲内） | 〃           |
| `## 現状`                   | `### 関連実装コード`     | 〃                                       | No / ファイル / メソッド / 概要 / 補足 の表        | 〃           |
| 〃                          | `### 関連テスト`         | 〃                                       | No / 分類 / ファイル / メソッド / 概要 / 補足 の表 | 〃           |
| 〃                          | `### 関連 Issue/PR`      | 〃                                       | No / 番号 / 状況 / 概要 / 補足 の表                | 〃           |
| 〃                          | `### 関連ドキュメント`   | 〃                                       | No / 分類 / ページ / 概要 / 補足 の表              | 〃           |
| 〃                          | `### 既存テスト実行結果` | テストが実行可能な場合                   | No / 分類 / ファイル / メソッド / 概要 / 結果 / 補足 の表 | 〃     |
| 〃                          | `### 再現手順`           | バグ報告の場合のみ                       | 番号付き箇条書き手順 + 再現結果                    | 〃           |
| `## システム要件（SA）`     | `### 機能要件`           | 〃                                       | No / カテゴリ / 要件 / 補足 の表（カテゴリは 編集機能/バリデーション/エラー表示/状態表示/レスポンシブ対応 など。エラーハンドリング・レスポンシブ対応の有無もここに含める） | issue-spec   |
| 〃                          | `### 非機能要件`         | 性能/セキュリティ/運用要件がある場合     | No / カテゴリ / 要件 / 補足 の表（カテゴリは 性能/セキュリティ/運用 など）                                            | 〃           |
| 〃                          | `### スコープ外`         | 誤解防止のため可能なら書く               | No / 項目 / 理由 / 補足 の表                                                                                          | 〃           |
| `## UI 設計`                | `### 画面構成`           | 画面ありの場合                           | No / 要素 / 種類 / 位置 / 説明 / 表示条件 / 必須 / 制限 / 初期値 / データソース / アクション / 補足 の表（データソースは `エンティティ.フィールド` 形式、アクションは日本語ドメイン記述。表に書き切れない内容は `※1` で表の直下に詳細記述。該当しない列は `-`） | issue-ui     |
| 〃                          | `### 画面遷移`           | 画面ありで複数画面が絡む場合             | Mermaid LR フローチャート                                                                                            | 〃           |
| 〃                          | `### モック`             | 画面ありの場合                           | No / 画面 / URL / 補足 の表                                                                                          | 〃           |
| `## システム方式設計（SS）` | `### 採用ライブラリ`     | 実装系                                   | No / ライブラリ / バージョン / 用途 / 補足 の表                                                                       | issue-arch   |
| 〃                          | `### コンポーネント分割` | 〃                                       | No / 新規/変更 / レイヤー / コンポーネント / 役割 / 補足 の表（レイヤーは フロント/バック/DB、新規/変更は 新規/変更/削除） | 〃           |
| 〃                          | `### データフロー`       | 複数コンポーネント間でデータが流れる場合 | Mermaid シーケンス図                                                                                                  | 〃           |
| `## PoC 結果`               | `### 検証したライブラリ` | 外部ライブラリ使用時                     | No / ライブラリ / バージョン / ライセンス / 用途 / 補足 の表                                                          | issue-arch または issue-ui |
| 〃                          | `### 動作確認結果`       | 〃                                       | No / ライブラリ / 検証項目 / 結果 / 補足 の表                                                                        | 〃           |
| 〃                          | `### 追記した Wiki`      | Wiki 追記/作成あり時                     | No / ページ / 内容 / 補足 の表（reset 時の巻き戻し履歴）                                                              | 〃           |
...
| 〃               | 使用ライブラリ一覧.md                    | 採用済みライブラリのインデックス（名前・バージョン・用途・`ライブラリ_{lib名}.md` へのリンク） |
| 〃               | ライブラリ_{lib名}.md                    | ライブラリごとの**プロジェクト固有の使い方**（独自ラッパー・統合パターン・ハマりポイント・バージョン明記） |
...
| サブセクション | 入力値                          | 概要                                                                                                            | 参照 Wiki                |
| -------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `### 画面構成` | 機能要件・既存画面              | **No / 要素 / 種類 / 位置 / 説明 / 表示条件 / 必須 / 制限 / 初期値 / データソース / アクション / 補足** の表（要素: ボタン・入力欄・ラベル・テーブル等／種類: button/input/table 等／位置: ヘッダー/フッター/フォーム本体/サイドバー 等／**データソース: `エンティティ.フィールド` 形式（DB 論理名 or データクラス名）。例: `ユーザ.名前` / `タスク.編集日付`**／**アクション: 日本語のドメイン記述で。例: 「クリックで詳細画面へ遷移」「クリックで保存」（API パスやコードは書かない）**／**表に書き切れない複雑なロジックは `※1` `※2` … の印を該当セルに入れ、表の直下に `※1: 〜` 形式で詳細を記述**／該当しない列は `-`） | 画面遷移図_*.md / データモデル一覧.md |
...
| サブセクション           | 入力値                                                               | 概要                                                                                                                            | 参照 Wiki                               |
| ------------------------ | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `### 採用ライブラリ`     | 本フェーズの PoC 結論・既存使用ライブラリ・library-finder の調査結果 | **No / ライブラリ / バージョン / 用途 / 補足** の表。新規採用は補足に選定理由                                                                | 使用ライブラリ一覧.md                   |
...
| `### 検証したライブラリ` | ユーザーが採用決定したライブラリ                                     | **No / ライブラリ / バージョン / ライセンス / 用途 / 補足** の表。採用決定した 1 ライブラリのみ記載、非決定の候補はコメントで議論し決定後に削除 | 使用ライブラリ一覧.md                   |
| `### 動作確認結果`       | 採用決定ライブラリの PoC 実行結果                                    | **No / ライブラリ / 検証項目 / 結果 / 補足** の表（成功条件・所要時間・所感など）+ 表後に最小再現コード（10〜30行程度）。非決定案の検証履歴は残さない | -                          |
| `### 追記した Wiki`      | このフェーズで新規作成・追記した Wiki ページ                         | **No / ページ / 内容 / 補足** の表（reset モニターが巻き戻し時に遡って削除するための履歴）                                                  | 使用ライブラリ一覧.md / ライブラリ_*.md |
...
4. **PoC 要否判定（ライブラリ系のみ対象）**: 採用候補が未経験で PoC が必要か判定（前述「PoC 要否判定カテゴリ」A〜E に該当するか）
   ...
   - **必要** → 以下の PoC サブステップを実施:
     ...
     - 4c. サブエージェント並列起動で各候補を独立検証（Wiki `使用ライブラリ一覧` を注入。担当 worktree で最小 PoC コードを書いて動作確認）
     - 4d. 各候補の動作結果・所感をコメントで投稿（議論用、決定後に削除）し採用判断を仰ぐ
     - 4e. ユーザー採用決定 → 採用案を本文 `## PoC 結果`（`### 検証したライブラリ` / `### 動作確認結果`）に記録
     - 4f. **Wiki 反映**: `使用ライブラリ一覧.md` に行追加・`ライブラリ_{lib名}.md` を新規作成または更新（プロジェクト固有の使い方・独自ラッパー・統合パターン・ハマりポイント・バージョン明記。公式 quickstart の単純コピペは書かない）→ 追記/新規作成した URL を `### 追記した Wiki` に記録
...
- 新規ページ追加時は、対応するインデックスページ（`使用ライブラリ一覧.md` など）も該当サブエージェントが自身で更新
...
2. 新規ページ追加時は、対応するインデックスページも該当 `doc-page-editor` が更新（例: 新規 `ライブラリ_xxx.md` 追加なら `使用ライブラリ一覧.md` にも行追加）
...
3. ユーザー応答ループ:
   ...
   - `フェーズ終了` 付与 → 次へ
4. **巻き戻し実行**:
   - 削除対象の Wiki ページを削除（`使用ライブラリ一覧.md` などインデックスからも該当行を削除）
...
- **議論が完了したコメントは「Resolve（Hide as resolved）」で隠す**（削除しない）
  - 後で AI 改善計画の素材として参照できるようにするため
  - 隠すには GitHub GraphQL API の `minimizeComment` mutation（`reason: RESOLVED`）を使う
- 返信は新しいコメントを作らず、同じコメントに追記（区切り線 `---` で分割）
- 全ての追記ブロック先頭に `@{送信者} → @{宛先}` で誰が誰宛かを明示
- AI が書いた追記には先頭に `🤖` を付ける（人間が書いた追記には付けない）

#### 宛先・送信者の書き方

- モニター宛: `@pr-impl` / `@pr-plan` / `@issue-arch` など（モニター名そのまま）
- ユーザー宛: GitHub ログイン名 `@shuhei-win` など
- 複数宛: カンマ区切り `@pr-impl, @shuhei-win`
- モニター名とユーザー名は被らないので `@` プレフィックスだけで識別可能

#### コメントフォーマット

1 つのコメント内で議論を続ける。AI/ユーザー が交互に区切り線で追記していく。
...
| 2   | AI → ユーザー | `🤖 @pr-impl-review → @shuhei-win` |
| 3   | AI → 複数宛 | `🤖 @pr-plan → @pr-impl, @shuhei-win` |
| 4   | ユーザー → AI | `@shuhei-win → @pr-impl-review` |
| 5   | ユーザー → 複数宛 | `@shuhei-win → @pr-impl, @pr-plan` |

#### ユーザーコメントへの対応

ユーザーのコメントは「指示」「質問」「提案」「議論」など意図が様々。ユーザーの言葉を鵜呑みにせず、AI が**自分で推論**してから対応する。

#### コメントの分類と対応パターン

| No  | ユーザーコメントの種類 | AI の対応 |
| --- | ----------------------- | --------- |
| 1   | 質問（「ここってどうなってる？」など） | 質問に答える追記のみ。本文は触らない。回答で議論完結したら Resolve |
| 2   | 明確かつ妥当な指示（AI も合意） | 即本文反映 → 「本文の `## XXX` を `Y` に修正しました」と短く追記 → Resolve |
| 3   | 不明確な指示・複数解釈可能な指示 | **ワンクッション**: 「この理解で合っていますか？こう書き換えます」と確認の追記 → ユーザー合意後に反映 → Resolve |
| 4   | 妥当性に疑問がある指示・提案 | 推論内容を返す: 「ご指摘の方法だと {懸念点}。代替案として {案} はいかがでしょうか？」 → 議論 → 結論が出たら反映 → Resolve |
| 5   | 議論・相談（「これどう思う？」） | AI の意見を返す → 結論が出たら反映 → Resolve |

#### 共通ルール

- 詳細をコメントに長々書かない。確定情報は本文に書く
- 本文反映後の応答は短く（「本文の `## XXX` を更新しました」程度）
- 元の本文記述は消してよい（常に「最新の確定版」に保つ）
- 部分書き換えは難しいので、各モニター担当セクションを丸ごと置換
- **コメントは個別に Resolve しない**（ユーザーが反映結果を確認できなくなるため）。Resolve はフェーズ終了時にまとめて行う（次項）
...
| 3   | 他のモニターが投稿したコメント                     | いいえ   | 無視（そのモニターのフェーズ終了時に Resolve される）      |
| 4   | ユーザー → 他のモニター宛のコメント                | いいえ   | 無視（例: pr-doc-review 中に発見した実装段階の指摘など）   |
...
| 5   | 「ここってどうなってる？」 | 1 質問 | 質問に回答のみ、本文は触らない | 「現状 `Service` クラスで処理しています。詳細は `クラス図_xxx.md` 参照」 |

※ いずれも Resolve はフェーズ終了時に一括で実施（個別 Resolve はしない）
...
issue-triage が起票直後に**全セクションの骨組み**を作成し、後続モニターが自分の担当セクションだけ上書き更新する。

記入例（題材: ユーザープロフィール編集画面を追加する）

```markdown
## 概要
<!-- issue-triage が記入：ユーザー入力の範囲内 -->
...
現在プロフィールは管理者画面からしか更新できず、ユーザーから「自分で更新したい」という要望が複数件寄せられている（#102, #145）。

## 現状
<!-- issue-triage が記入：コードベース調査結果。各サブセクションは表で整理 -->
...
| No | 番号    | 状況   | 概要                                | 補足                 |
| -- | ------- | ------ | ----------------------------------- | -------------------- |
| 1  | #102    | open   | 同じ要望、本 Issue に集約           | 集約後 close 予定    |
| 2  | #145    | closed | 過去の同様要望（実装見送り）        | 当時の判断理由を参考 |
...
| No | 分類      | ページ                       | 概要                          | 補足                    |
| -- | --------- | ---------------------------- | ----------------------------- | ----------------------- |
| 1  | Wiki      | `クラス図_user.md`           | User 周辺のクラス構造         | 更新対象                |
| 2  | 〃        | `APIエンドポイント一覧.md`   | 既存 API 一覧（PUT 追加対象） | PUT 追加で更新が必要    |
| 3  | 〃        | `データモデル一覧.md`        | User DTO 既存定義             | bio カラム追記が必要    |
...
### 機能要件
<!-- issue-spec が記入。エラーハンドリング・バリデーションもここに含める -->

| No | カテゴリ         | 要件                                                              | 補足                              |
| -- | ---------------- | ----------------------------------------------------------------- | --------------------------------- |
| 1  | 編集機能         | ログイン中ユーザーが氏名・自己紹介・アバター画像を編集できる      | -                                 |
| 2  | 〃               | 保存ボタンで編集内容を確定保存できる                              | -                                 |
| 3  | 〃               | キャンセルボタンで編集を破棄して詳細画面に戻れる                  | -                                 |
| 4  | バリデーション   | アバター画像は 5MB 以下に制限                                     | クライアント側で事前チェック      |
| 5  | 〃               | 自己紹介は 500 文字以内に制限                                     | 入力時に文字数カウンタ表示        |
| 6  | エラー表示       | 5MB 超のアバター画像はエラーメッセージを表示                      | 保存ボタン押下前にブロック        |
| 7  | 〃               | 自己紹介が 500 文字を超えた時にエラーメッセージを表示             | 入力中にリアルタイム表示          |
| 8  | 状態表示         | 保存処理中はボタンが無効化されインジケーターを表示                | 二重送信防止も兼ねる              |
| 9  | レスポンシブ対応 | デスクトップ・タブレット・スマホで利用可能                        | ブレークポイント 768px / 1024px   |

### 非機能要件
<!-- issue-spec が記入、当てはまるものがある時のみ -->
...
### スコープ外
<!-- issue-spec が記入、誤解防止のため可能なら -->

| No | 項目                  | 理由                                  | 補足                  |
| -- | --------------------- | ------------------------------------- | --------------------- |
| 1  | パスワード変更        | 別フロー（メール認証込み）が必要      | 別 Issue 起票予定     |
| 2  | メールアドレス変更    | メール認証フローが必要                | 別 Issue              |
| 3  | プロフィール公開設定  | 将来要件、本リリース対象外            | -                     |

## PoC 結果
<!-- issue-arch / issue-ui が記入、ライブラリ選定で PoC が必要な場合のみ -->

### 検証したライブラリ

| No | ライブラリ          | バージョン | ライセンス | 用途                | 補足          |
| -- | ------------------- | ---------- | ---------- | ------------------- | ------------- |
| 1  | `react-image-crop`  | v11.0.5    | MIT        | 画像トリミング機能  | 採用決定済み  |

### 動作確認結果

| No | ライブラリ          | 検証項目                    | 結果                              | 補足              |
| -- | ------------------- | --------------------------- | --------------------------------- | ----------------- |
| 1  | `react-image-crop`  | 5MB 画像の読み込み          | 平均 280ms                        | 許容範囲          |
| 2  | 〃                  | トリミング処理              | クライアント側で完結              | サーバー負荷なし  |
| 3  | 〃                  | iOS Safari 動作確認         | 問題なし                          | -                 |

最小再現コード:

```ts
// react-image-crop の最小利用例
```
...
| No | ページ                        | 内容                       | 補足                          |
| -- | ----------------------------- | -------------------------- | ----------------------------- |
| 1  | `ライブラリ_react-image-crop.md` | 新規作成                | 採用ライブラリの使い方        |
| 2  | `使用ライブラリ一覧.md`       | 行追加                     | 上記新規ページへのリンク追加  |
...
| No | 要素                | 種類           | 位置          | 説明                          | 表示条件        | 必須 | 制限         | 初期値                | データソース            | アクション                       | 補足              |
| -- | ------------------- | -------------- | ------------- | ----------------------------- | --------------- | ---- | ------------ | --------------------- | ----------------------- | -------------------------------- | ----------------- |
| 1  | 氏名                | input          | フォーム本体  | テキスト入力                  | 常時            | 必須 | -            | 現プロフィール値      | `ユーザ.名前`           | -                                | -                 |
| 2  | 自己紹介            | textarea       | 〃            | 文字数カウンタ付き            | 常時            | 任意 | 500 文字以内 | 〃                    | `ユーザ.自己紹介`       | -                                | -                 |
| 3  | アバター画像        | drag-and-drop  | 〃            | ドラッグ&ドロップ + トリミング | 常時            | 任意 | 5MB 以下     | 〃                    | `ユーザ.アバター画像`   | クリックでファイル選択ダイアログ | -                 |
| 4  | 保存ボタン          | button         | フッター      | primary スタイル              | 常時            | -    | -            | -                     | -                       | クリックで保存して詳細画面へ遷移 ※1 | 保存中は disabled |
| 5  | キャンセルボタン    | button         | 〃            | secondary スタイル            | 常時            | -    | -            | -                     | -                       | クリックで詳細画面へ遷移 ※2      | -                 |
| 6  | 保存中インジケータ  | spinner        | 保存ボタン上  | スピナー表示                  | 保存処理中のみ  | -    | -            | -                     | -                       | -                                | 二重送信防止兼用  |

※1: 保存時の処理フロー
1. クライアント側でバリデーション（文字数・画像サイズ）
2. バリデーション失敗時はエラーメッセージを各入力欄の下に表示してリターン
3. 成功時は保存中インジケーターを表示してプロフィール更新リクエストを送信
4. 成功レスポンスを受け取ったらプロフィール詳細画面へ遷移
5. 失敗時はトーストでエラーメッセージを表示し編集状態を維持

※2: キャンセル時、編集内容に変更があれば「変更を破棄しますか？」の確認モーダルを表示してから遷移

### 画面遷移

```mermaid
flowchart LR
  view[プロフィール詳細] -->|編集ボタン| edit[編集画面]
  edit -->|保存成功| view
  edit -->|キャンセル| view
```
...
| No | 画面             | URL                                                | 補足             |
| -- | ---------------- | -------------------------------------------------- | ---------------- |
| 1  | プロフィール編集 | https://example-mock.example.com/profile-edit     | デスクトップ版   |

## システム方式設計（SS）
<!-- issue-arch が記入、実装系のみ -->

### 採用ライブラリ

| No | ライブラリ          | バージョン | 用途                  | 補足                |
| -- | ------------------- | ---------- | --------------------- | ------------------- |
| 1  | `react-image-crop`  | v11.0.5    | 画像トリミング機能    | PoC 採用済          |
| 2  | `react-hook-form`   | 既存       | フォーム管理          | 既存採用ライブラリ  |

### コンポーネント分割

| No | 新規/変更 | レイヤー | コンポーネント                       | 役割                              | 補足                       |
| -- | --------- | -------- | ------------------------------------ | --------------------------------- | -------------------------- |
| 1  | 新規      | フロント | `ProfileEditScreen`                  | 編集画面                          | -                          |
| 2  | 〃        | 〃       | `AvatarCropModal`                    | アバター画像のトリミングモーダル  | -                          |
| 3  | 〃        | バック   | `PUT /api/users/{id}` エンドポイント | プロフィール更新 API              | -                          |
| 4  | 〃        | 〃       | `UserService.updateProfile()`        | プロフィール更新ビジネスロジック  | 既存クラスへのメソッド追加 |
| 5  | 変更      | DB       | `users.bio` カラム                   | 自己紹介保存（VARCHAR(500)）      | 追加マイグレーション       |
...
sequenceDiagram
  ...
  participant FE as ProfileEditScreen
...
| 4  | ⬜   | `ライブラリ_react-image-crop.md`    | -                  | 新規ページ作成                            | 採用ライブラリの使い方        |
| 5  | ⬜   | `使用ライブラリ一覧.md`             | -                  | `ライブラリ_react-image-crop.md` 行追加   | 要件由来                      |
