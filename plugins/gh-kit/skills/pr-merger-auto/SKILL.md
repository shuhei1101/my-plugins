...
description: マージ可能 ラベル付き Ready PR を 1 件ずつ直列でマージする
disable-model-invocation: true
---

# pr-merger-auto

`マージ可能` ラベル付き Ready PR をキューとして 1 件ずつ消化する。
...
対象条件: `マージ可能` ラベル付きの Ready（非 Draft）PR（`処理中` 付きは除外）。
...
| verdict | 動作 |
|---|---|
| merged | `gh pr edit {N} --remove-label "$GH_KIT_LABEL_PROCESSING" --remove-label "$GH_KIT_LABEL_APPROVED_MERGE_OK"`（マージは pr-merger が実施済み）+ `gh issue edit "$ISSUE_N" --remove-label "$GH_KIT_LABEL_PROCESSING_PR_REVIEWER"` |
