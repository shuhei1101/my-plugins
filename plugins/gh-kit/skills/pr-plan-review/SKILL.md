...
| 2 | `blocker` / `major` を含む | `needs-revision` | ステップ 7 へ（`$GH_KIT_LABEL_CONFIRM_PR_PLANNER` ラベル付与） |
...
## ステップ 7: 不合格時 — 確認:pr-planner ラベル付与

```bash
gh pr edit {N} --add-label "$GH_KIT_LABEL_CONFIRM_PR_PLANNER"
```
