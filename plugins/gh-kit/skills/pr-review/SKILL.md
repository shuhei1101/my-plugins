...
# コンフリクト通知コメントをテンプレートから生成して投稿
# テンプレート: ${CLAUDE_PLUGIN_ROOT}/templates/コンフリクト通知コメント.j2
# {head_branch}, {base_branch}, {conflict_files}, {conflict_reason} を埋めて gh pr comment で投稿
gh pr comment {PR_NUMBER} --body "$(cat <<'COMMENT'
> ⚠️ AI 自走解消不可 — コンフリクトが発生しています

## 状況

`{HEAD_BRANCH}` を `{BASE_BRANCH}` にマージしようとしたところ、コンフリクトが発生し AI による自動解消ができませんでした。
...
- [ ] A: `{HEAD_BRANCH}` 側（この PR の変更）を優先する
- [ ] B: `{BASE_BRANCH}` 側（master の変更）を優先する
- [ ] C: 両方を活かして手動で統合する
- [ ] D: この PR をいったんクローズし、改めて作り直す
- [ ] その他（コメントで指示してください）

**選択後**: コメントで「A を採用」などと返信し、`needs-ai-review` を再付与してください。
