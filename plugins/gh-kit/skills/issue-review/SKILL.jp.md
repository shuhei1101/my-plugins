...
ステップ 1 で取得した `レビュー結果コメント.j2` に沿って対応案 / 質問 / 分割提案 / 影響範囲を書く。
質問・分割提案がなければ該当セクションごと省略。

**「対応案」セクションの省略ルール**: Issue 本文（またはステップ 4 で投稿した本文補完コメント）に「対応案」セクションが既に存在する場合、レビュー結果コメントの「対応案」セクションを**省略する**。重複して記載しない。
