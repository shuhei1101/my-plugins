...
処理をサブエージェントに委譲する場合は `🤖` マーカーを付ける（詳細は **[## サブエージェント]** 参照）:

```markdown
#### 処理
1. 🤖 既存スキルを Glob し description を収集する（戻り値: `[{name, description}]`）
→ ステップ 2 へ
```
...
ステップ内の処理を **サブエージェント** に委譲できる。委譲したいステップの処理には `🤖` マーカーを付け、
戻り値の形式を括弧内に明記する（上の「ステップ構造」のコード例も参照）。
...
- **完了を必ず待つ**: 完了を待たないケースは基本ない。複数並列実行しても最終的には全完了を待つ。
- **コンテキスト**: サブエージェントはコンテキストが空の状態で起動する。必要な情報はプロンプトで明示的に渡す。
