...
指示ファイルを作成・編集したら、`claude-kit:version-sync` が古い成果物を検出できるよう出自
メタデータを刻む。完全な仕様（拡張子別書式・version の出どころ・配置・JP ミラー警告）は
**`provenance.md`** にあり、スタンプ対象ファイルを編集するとこのガイドと一緒に注入される —
それに従ってスタンプを直接書く。
