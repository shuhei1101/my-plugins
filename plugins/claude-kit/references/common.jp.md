...
`<!-- This file is a Japanese mirror. ... -->` で始める必要がある（下記の出自付与ステップが
`*.jp.md` には自動で追加する）。
...
## 出自スタンプ（任意のファイルを作成・編集した後は必須）

生成・編集したすべての指示ファイルには、`claude-kit:version-sync` が後で古いプラグイン
バージョンで生成されたファイルを検出できるよう、出自スタンプを必ず付けること。

**手順** — 作成または書き直す各ファイルに対して実行:
...
1. スタンプ文字列を取得:
   ...
   `<skill-or-source>` には対応するソース名を使う（例: `skill-creator`, `rule-creator`,
   `hook-creator`, `claude-creator`, `plugin-creator`）。注入されたリファレンスから直接
   ファイルを執筆する場合（creator スキルを使わない場合）でも、スタンプを付けること —
   ファイル種別に最も合うソースを選ぶ。

2. 返ってきたコメントを **YAML frontmatter の直後**（閉じ `---` の直後）に挿入する。
   frontmatter が無い場合は最上部に挿入する。決して `---` の前には挿入しないこと —
   Markdown の frontmatter 検出が壊れる。

3. ファイルが既に `<!-- Generated by`（`*.py` / `*.sh` / `*.yaml` の場合は `# Generated by`）で
   始まっている場合は、その 1 行を重複させず置き換える。

JP ミラーファイル（`*.jp.md`）の場合、返される文字列にはメタデータ行と JP ミラー警告行の
両方が含まれる — 両方をそのまま貼り付ける。`mark-generated` が空文字列を返した場合
（例: `*.json` はコメントを持てない）、黙ってスキップする。
