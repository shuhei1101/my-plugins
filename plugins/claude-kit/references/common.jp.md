...
### まず `CLAUDE_KIT_JP_MIRROR` を確認する

ファイルを作成する前に、次のコマンドを実行して JP ミラーモードを確認すること:
...
| 値 | 動作 |
|---|---|
| `true`（デフォルト / 未設定） | 別途 `.jp.md` ミラーファイルを作成する（以下のワークフローに従う） |
| `false` | `.jp.md` を作らず、本体 `.md` ファイルを日本語で直接書く |

**`CLAUDE_KIT_JP_MIRROR=false` の場合**: 以下の表の「英語ファイル」をそのまま日本語で書く。
`.jp.md` ミラーは作成しない。

---

すべてのファイルには対応する JP ミラーが必要（`CLAUDE_KIT_JP_MIRROR=true` の場合）:
