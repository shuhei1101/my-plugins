...
the warning comment `<!-- This file is a Japanese mirror. ... -->` (the provenance step below
adds it automatically for `*.jp.md`).
...
## Provenance stamping (required after creating or editing any file)

Every generated/edited instruction file MUST carry a provenance stamp so
`claude-kit:version-sync` can later detect files produced by an older plugin version.

**Procedure** — run for each file you create or rewrite:

1. Get the stamp string:

   ```
   /claude-kit:mark-generated --plugin claude-kit --skill <skill-or-source> --file <generated-file-path>
   ```

   Use the matching source name for `<skill-or-source>` (e.g. `skill-creator`, `rule-creator`,
   `hook-creator`, `claude-creator`, `plugin-creator`). When you author the file directly from
   an injected reference (no creator skill), still stamp it — pick the source that best matches
   the file type.

2. Insert the returned comment **right after the YAML frontmatter** (immediately after the closing
   `---`), or at the very top if there is no frontmatter. Never insert before `---` — it breaks
   Markdown frontmatter detection.

3. If the file already starts with `<!-- Generated by` (or `# Generated by` for `*.py` / `*.sh` /
   `*.yaml`), replace that single line instead of duplicating it.

For JP mirror files (`*.jp.md`), the returned string includes both the metadata line and the
JP-mirror warning line — paste both unchanged. If `mark-generated` returns an empty string
(e.g. `*.json` cannot carry comments), skip silently.
