...
Japanese mirror: `references/common/共通ガイド.jp.md`

For type-specific details, read the dedicated reference:
- Rules: `references/claude-md/記述ルール.md`
- Skills: `references/skill/スキル.md`
- Hooks: `references/hook/フック.md`
...
the file Claude actually loads, so the two must never drift.

**JP mirror warning comment.** Every JP mirror must start with this exact warning comment
(stamped when you write the file):

```
<!-- This file is a Japanese mirror of {source}.md. When updating the English original, update this file too. -->
```

Replace `{source}` with the English original's name (e.g. `SKILL` for `SKILL.jp.md`). Skip this
for `.json` files, which cannot carry comments. If an old stamp line is present, replace it
