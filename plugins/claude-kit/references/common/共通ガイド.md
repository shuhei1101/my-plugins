<!-- This file is a Japanese mirror of 共通ガイド.md. When updating the English original, update this file too. -->
# Claude 設定 共通ガイド

すべての creator スキルと `claude-refactor` のための共有リファレンス。
英語原本: `references/common/共通ガイド.md`

種別ごとの詳細は、各専用リファレンスを参照:
- ルール: `references/claude-md/記述ルール.md`
- スキル: `references/skill/スキル.md`
- フック: `references/hook/フック.md`
- CLAUDE.md: `references/claude-md/CLAUDE-md記述ガイド.md`
...
**JP ミラー警告コメント。** すべての JP ミラーは、ファイルを書く際に以下の警告コメントで始める:

```
<!-- This file is a Japanese mirror of {source}.md. When updating the English original, update this file too. -->
```

`{source}` は英語原本の名前に置き換える（例: `SKILL.jp.md` なら `SKILL`）。コメントを持てない
`.json` ファイルではスキップする。既存のスタンプ行があれば、追記せず置き換える。
