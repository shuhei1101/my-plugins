...
|---|---|---|
| H1 title + opening paragraph | Plugin name + 1–3 sentence description | **Always required** |
| `## Skills` | Table: skill / description / when invoked | **Always required** |
...
List every skill the plugin ships. Include what triggers it — explicit user call, a dispatch hook,
or an autonomous invocation pattern.
...
| `{plugin}:{skill-2}` | {one-line purpose} | Dispatched by the `{hook-name}` hook when ... |
```
...
| Skill | Description | When Invoked |
|---|---|---|
| `{plugin}:{skill}` | {purpose} | {explicit / hook dispatch / auto} |
