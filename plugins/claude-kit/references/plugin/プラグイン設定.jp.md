<!-- This file is a Japanese mirror of プラグイン設定.md. When updating the English original, update this file too. -->
# プラグイン Config スキルガイド

Claude Code プラグインに **config スキル** を追加する方法。config スキルとは、`settings.json` を
手動で編集しなくても、プラグインの env 変数制御の機能をユーザーがトグルできるスキルです。

英語正本: `references/plugin/プラグイン設定.md`
...
プラグインはオプトイン / オプトアウトの挙動を環境変数で制御します（`環境変数.md` 参照）。
