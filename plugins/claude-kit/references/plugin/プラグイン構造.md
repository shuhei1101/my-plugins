...
Read `共通ガイド.md` alongside it.
Japanese mirror: `references/plugin/プラグイン構造.jp.md`
...
| Reference | `references/plugin/セットアップウィザード.md` — full flow, skeleton, and checklist |

See `セットアップウィザード.md` for the detailed authoring guide, skeleton, and SessionStart-hook implementation.
...
- **Creating**: generate the directory structure above. Add agents/hooks/MCP dirs only if requested.
  Create `CLAUDE.md` and `CLAUDE.jp.md` following `プラグインCLAUDE-md.md` — every plugin requires them.
...
Follow the checklist in `バージョン同期.md`. The short rule: the three files must always carry the
...
Full authoring guide: `プラグインCLAUDE-md.md`.
...
`env` block and read with `os.environ` (full guide: `環境変数.md`). When a plugin reads any env var,
...
(e.g. `PY_KIT_INJECTION_TTL`). See `プラグインCLAUDE-md.md` for the table format.
