...
Japanese mirror: `references/plugin/セットアップウィザード.jp.md`

Counterpart to `plugin-migrate` (version sync) — `setup-wizard` covers initial setup.
Read `共通ガイド.md` and `スキル.md` alongside this guide.
...
Create the JP mirror `SKILL.jp.md` simultaneously (see `共通ガイド.md` JP/EN mirror rules).
