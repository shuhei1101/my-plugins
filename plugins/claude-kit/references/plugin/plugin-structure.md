...
Japanese mirror: `references/plugin/plugin-structure.jp.md`
...
Follow the checklist in `version-sync.md`. The short rule: the three files must always carry the
same version number — never let them drift.
