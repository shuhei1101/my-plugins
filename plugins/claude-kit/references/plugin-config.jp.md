...
`multiSelect: false` で `AskUserQuestion` を呼ぶ：

```yaml
question: "設定する env 変数を選択"
header:   "env 変数"
options:
  - label: "[{状態}] {VAR_NAME_1}"   description: "{機能の説明}"
  - label: "[{状態}] {VAR_NAME_2}"   description: "{機能の説明}"
  - label: "[{状態}] {VAR_NAME_3}"   description: "{機能の説明}"
  - label: "完了（設定を終了）"         description: "ループを終了して変更結果を表示"
```

オプションリストは **ユーザー向け変数 3 つ + 完了** が上限。トグルが多い場合は
よく使う 3 つをリストに入れ、残りは「その他」自由入力で受け付ける（フルリストで検証してから進む）。

完了を選択した場合 → Step 5 へジャンプ。
