...
| # | バージョン | 概要 |
|---|---|---|
| 1 | `3.48.0` | `claude-kit:config` スキルを追加 — `CLAUDE_KIT_JP_MIRROR` / `CLAUDE_KIT_INJECTION_LANG` / `CLAUDE_KIT_INJECTION_TTL` を番号付きリストループと `AskUserQuestion` でインタラクティブに設定 |
| 2 | `3.47.0` | `references/jinja2/templates.md` を追加 — Markdown を出力する Jinja2 テンプレートのオーサリングルール；`**/hooks/templates/*.j2` 編集時に自動注入 |
