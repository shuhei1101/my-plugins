...
The authoring guides for each instruction-file type live in `references/` (`common.md`, `provenance.md`,
...
- **Provenance**: stamping is documented in `references/provenance.md`, injected alongside the
  authoring guide whenever you write a stampable file — write the stamp directly (no skill call).
  The `mark-generated` skill survives only as a **thin wrapper** over `provenance.md`, for the
  fallback cases the hook cannot reach (helper-script output, non-matched file types). `version-sync`
  consumes these stamps.
...
in PR161). `.j2` stamping is covered by the `**/*.j2 → provenance.md` injection rule; JP-mirror sync
is enforced by the project's `*-jp-mirror-sync` rules.
