...
| # | Version | Summary |
|---|---|---|
| 1 | `3.52.0` | Remove `claude-kit:config` skill |
| 2 | `3.51.0` | Remove `claude-kit:setup-wizard` skill and `SessionStart` hook (`setup_check.py`) |
| 3 | `3.49.1` | Remove branch-check step (master/main guard) from `plugin-migrate` — redundant with the work harness UserPromptSubmit hook |
| 4 | `3.48.0` | Reorganize `references/` into role-based subfolders (`common/`, `skill/`, `hook/`, `claude-md/`, `plugin/`); add `plugin/バージョン同期.md`; inject version-sync reminder on `plugins/*/CLAUDE.md` edits |
| 5 | `3.47.0` | Add `references/jinja2/templates.md` — authoring rules for Jinja2 templates that emit Markdown; auto-injected on `**/hooks/templates/*.j2` edits |
| 6 | `3.46.0` | Add `references-edit-guard` PreToolUse hook — reminds to update `_index.yaml` / `_injection_rules.yaml` when editing `references/` |
| 7 | `3.44.0` | Add `${CLAUDE_KIT_JP_MIRROR}` env var — when `false`, skip `.jp.md` mirrors and write the main file in Japanese |
| 8 | `3.43.0` | Rename meta-YAML files under `references/` with `_` prefix; update plugin-name docs (PR179) |
| 9 | `3.42.0` | Add `${CLAUDE_KIT_INJECTION_DISABLE}` kill switch env var |
