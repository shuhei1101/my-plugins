...
#### ユーザ確認
(If this step requires user input before proceeding, list numbered choices here.
When choices are present, Claude MUST stop and wait for the user's selection.
If no confirmation is needed, write "なし".)

Example 1 — multiple choices (e.g., PR name confirmation):
**\<Confirmation title: describe what the user is being asked in one line\>**
1. <auto-generated PR name A> (recommended) → create PR and proceed to next step
2. <auto-generated PR name B> → create PR and proceed to next step
3. <auto-generated PR name C> → create PR and proceed to next step
4. 手動で入力する → create PR with entered name and proceed to next step

Example 2 — implementation review (OK / NG):
**\<Confirmation title: describe what the user is being asked in one line\>**
1. 確認OK → proceed to next step
2. 確認NG・要望を伝える → redo the implementation
