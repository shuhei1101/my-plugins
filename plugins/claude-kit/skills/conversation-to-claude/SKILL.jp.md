...
1. セッション全体の会話を振り返り、以下を抽出する:
   ...
   **F. プロジェクト固有の用語**（`glossary` ルール向け・自動追記）
...
1. `.claude/rules/glossary.md` を読み込む（存在しない場合は新規作成）
2. 各用語を適切なカテゴリ（H2セクション）に分類する
3. 既に記載済みの用語はスキップし、新規用語のみ追記する
4. 追記フォーマット:
   ...
   | 用語 | 説明 |
   |---|---|
   | {term} | {1〜2文の定義。会話の文脈から推測して書く} |
   ```

5. ユーザーへの報告は不要（サイレントに実行する）

> **注意**: glossary は常時読み込まれるルールのため、簡潔に保つ。
> 用語の説明は1〜2文以内。書くことに慎重になること。

→ ステップ 3 へ

#### 出力

- `.claude/rules/glossary.md` が更新済み（または変更なし）
...
| glossary | `.claude/rules/glossary.md`（常時読み込み） | プロジェクト固有の用語定義（自動追記） |
