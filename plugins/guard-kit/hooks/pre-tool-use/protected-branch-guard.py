...
Edit・Write ツール呼び出しを阻止して /gh-kit:start の使用を促す。
    ...
    return {
        "hookSpecificOutput": {
            ...
            "permissionDecisionReason": (
                ...
                "必ず /gh-kit:start スキルを使用してワークツリーを作成し、そこで作業を行ってください。\n\n"
                "ユーザーに以下を日本語で伝えてください：\n"
                f"「{branch} ブランチへの直接編集はできません。"
                "/gh-kit:start を実行してワークツリーを作成してから作業を開始してください。」"
            ),
            "additionalContext": (
                f"保護ブランチ '{branch}' への直接編集がブロックされました。\n"
                "- 禁止理由: このプロジェクトはワークツリーを使った分離作業を必須としています\n"
                "- 対処法: `/gh-kit:start` スキルを実行してブランチとワークツリーを作成する\n"
