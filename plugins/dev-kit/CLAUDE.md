...
ルール自動注入は `hooks/rules/`（`inject_rules.py` + ルール `.md` 群 + 実行時生成の `cache.yaml`）に集約。
その他のフックは `hooks/scripts/` 配下、共通ヘルパは `_common.py`。

| フック | トリガー | 用途 |
|---|---|---|
| `rules/inject_rules.py` | PreToolUse(Edit/Write/Read) | ルール自動注入（各ルールファイルのフロントマターで制御） |
| `scripts/references_edit_guard.py` | PreToolUse(Edit/Write) | ルール編集時にフロントマター記述漏れをリマインド |
...
| `${DEV_KIT_INJECTION_DISABLE}` | キルスイッチ — truthy で全ルール注入を停止 | - true<br>- **false** |
| `${DEV_KIT_INJECTION_TTL}` | ルールのトークンキャッシュ TTL。秒（整数） | **3600** |

## ルール構造

```
hooks/rules/
├── inject_rules.py  # フロントマター判定 + 注入スクリプト
...
各ルールファイルの先頭に YAML フロントマターでトリガーを記述する:
...
`inject_rules.py` は `hooks/rules/` 配下の `.md` を走査してフロントマターを `cache.yaml` にキャッシュし、以後はそれを読む。
**ルールを更新したら `cache.yaml` を削除すれば再生成される。**
発火は Edit / Write / Read すべて。注入は `~/.claude/tokens/dev-kit/{session_id}.yaml` の TTL トークンで二重注入を防ぐ。

## Changelog

| Version | Date | Summary |
|---|---|---|
| 4.18.0 | 2026-06-10 | 注入フォルダを `hooks/rules/` に改称し references サブフォルダを廃止（直下展開）。`main.py` → `inject_rules.py`。`tools` キーを廃止し Edit/Write/Read すべてで発火 |
