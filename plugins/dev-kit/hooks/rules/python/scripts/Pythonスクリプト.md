...
def process(input_paths: Path, output_paths: Path) -> None:
...
1. **モジュール docstring**: 1 行目で何をするか、続けて詳細と使い方の例。
   Usage は**インデントなし**でコマンドを書きそのままコピペ実行可能にし、コマンドの**上**に `#` で説明を添える。任意引数は `[角括弧]` で示す（例: `python my-script.py --input data.csv [--verbose]`）
