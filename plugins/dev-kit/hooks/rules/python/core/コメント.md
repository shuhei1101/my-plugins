...
Next の `frontend/conventions/コメント.md` と同じ思想で Python に適用する。
...
AI と共同で書く前提なので、**多めに書く**。

コメント量は**コードの非自明さに従い、レイヤーで分けない** — ユースケースを束ねるサービスも、機能内部のヘルパーも、末端のユーティリティも同じように書く（「高レイヤーだから濃く」ではない）。
...
ブロックマーカーだけでは足りない。**各ステップが何をしてなぜそうするのか**、特に非自明な箇所に一言コメントを足す。下の例はたまたまサービス関数だが、内部ヘルパーやユーティリティもまったく同じように書く。
...
- `core/命名規則.md` — 命名規約
- `core/型ヒント.md` — 型ヒントの書き方
- `architecture/TypeScriptスタイル適用.md` — DTO 定義の実例
