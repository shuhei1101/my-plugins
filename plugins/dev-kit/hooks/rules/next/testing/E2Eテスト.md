...
export const loginAsTestUser = async (page: Page) => {
  await page.context().addCookies([
    { name: "session_token", value: TEST_SESSION_TOKEN, domain: "localhost", paths: "/" },
  ])
}
```
...
export default async (config: FullConfig) => {
  ...
  await ctx.storageState({ paths: "tests/e2e/.auth/user.json" })
  await browser.close()
}
```
...
## メインのdev serverが動いている状態でのE2E実行 — Worktreeを使う

Next.jsには「**同一ディレクトリでdev serverは1つだけ**」という制約がある。
開発中にメインのdev serverが動いているリポジトリから別ポートでE2E用サーバーを起動しようとすると
`"Another next dev server is already running"` エラーになる。

**worktree（別ディレクトリ）から起動することで、E2E専用のdev serverを独立して起動できる。**

### ポートの設計方針

- メインと被らない専用ポートをE2Eに割り当て、`playwright.config.ts` の `webServer` に設定する
- `reuseExistingServer: !process.env.CI` で既存サーバーを再利用できるが、
  そのURLに何も動いていなければ起動コマンドを実行しようとするため、
  同一ディレクトリでは「Another server is already running」エラーになる
- worktreeは別ディレクトリなので、独立してサーバーを起動できる

### 実行手順

```bash
# 1. E2E用worktreeを作成（まだなければ）
git worktree add ../my-repo-wt-e2e -b e2e/run

# 2. worktreeに移動
cd ../my-repo-wt-e2e

# 3. 依存をインストール（worktreeにはnode_modulesがないため）
npm install

# 4. Playwrightブラウザをインストール（初回のみ）
