...
- IME 入力中（`composing === true`）の Enter は **無視必須**（日本語変換確定との衝突防止）。画面ごとに手書きせずこの共通コンポーネントを使う
- 重複タグ・空文字（trim 後）は追加しない
- onBlur でも commit（モバイルでフォーカスアウト時に失われるのを防ぐ）
- × でタグ削除

## 関連 references

- `frontend/コンポーネントカタログ.md`
- `frontend/フォームコンポーネント.md` — `<FormField>` 内での利用例
- shadcn `<Badge>` `<Input>`
