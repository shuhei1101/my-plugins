...
- **第一選択は Server Action 直接呼び**（`useTransition`、`actions.ts`）。楽観更新が必要なときだけ useMutation
- mutation エラーは `onError` で toast.error（または `handleAppError`）、成功時は `invalidateQueries` + toast.success
