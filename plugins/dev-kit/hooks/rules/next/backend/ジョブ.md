...
```json
{
  "crons": [
    ...
    { "path": "/api/v{N}/cron/weekly-cleanup", "schedule": "0 3 * * 1" }
  ]
}
```

```ts
// app/api/v{N}/cron/daily-summary/route.ts
...
`CRON_SECRET` は Vercel ダッシュボードで設定。Vercel Cron は自動的に `Authorization: Bearer ${CRON_SECRET}` を付ける。サービス資格情報は環境変数経由で渡す（`PUBLIC_` prefix 禁止）。
...
await qstash.publishJSON({
  url: "https://example.com/api/v{N}/jobs/send-email",
...
```ts
// app/api/v{N}/jobs/send-email/route.ts
