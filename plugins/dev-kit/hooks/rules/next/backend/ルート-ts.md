...
export async function GET(_: NextRequest, ctx: RouteContext<'/api/v{N}/resources/[id]'>) {
  return withRouteErrorHandling(async () => {
...
export async function PATCH(request: NextRequest, ctx: RouteContext<'/api/v{N}/resources/[id]'>) {
  return withRouteErrorHandling(async () => {
    const { id } = await ctx.params
    const { db, userId } = await getAuthContext()
    const body = await request.json()
...
export async function DELETE(_: NextRequest, ctx: RouteContext<'/api/v{N}/resources/[id]'>) {
  return withRouteErrorHandling(async () => {
    const { id } = await ctx.params
    const { db, userId } = await getAuthContext()
...
- `RouteContext<'/api/v{N}/resources/[id]'>` の型ヘルパーを使う
