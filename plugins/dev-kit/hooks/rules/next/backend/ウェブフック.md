...
```ts
// app/api/v{N}/webhooks/stripe/route.ts
...
Webhook は素早く 2xx を返すべき。重い処理は **バックグラウンドジョブに enqueue**（`backend/ジョブ.md`）して即時 200 を返す:
...
```ts
// app/api/v{N}/webhooks/stripe/service.ts
...
- **Stripe**: `stripe listen --forward-to localhost:3000/api/v{N}/webhooks/stripe`
