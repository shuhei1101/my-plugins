...
```ts
// app/api/v{N}/resources/types.ts
...
```ts
// app/api/v{N}/resources/query.drizzle.ts
...
```ts
// app/api/v{N}/resources/db.drizzle.ts
...
ロールバックは `data/dev/` 全体のスナップショット復元。同時実行は in-process ロックで直列化（単一プロセス・ローカル開発専用。分散ロックは想定しない）。

```ts
// app/api/v{N}/resources/query.yaml.ts
...
```ts
// app/api/v{N}/resources/db.yaml.ts
...
```ts
// app/api/v{N}/resources/query.ts
...
```ts
// app/api/v{N}/resources/service.ts
