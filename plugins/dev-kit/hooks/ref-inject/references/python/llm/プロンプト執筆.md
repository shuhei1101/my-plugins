...
`llm/コストとキャッシュ.md` に書いた **「固定値は上、動的値は下」** をディレクトリ構造で表現:
...
# ----- 部品一覧 -----
parts:
  - paths: modes/notify/static/role.md
    kind: static
    role: system
    category: role
  - paths: modes/notify/static/policy_chat.md
    kind: static
    role: system
    category: policy
  - paths: modes/notify/static/output_csv_format.md
    kind: static
    role: system
    category: output_format
  - paths: modes/notify/dynamic/situation.j2
    kind: dynamic
    role: user
    category: situation
...
parts:
  - paths: characters/akane/static/personality.md
    category: character
  - paths: characters/akane/static/speaking_スタイル.md
    category: character
```

ローダーは同じ category の部品連続をまとめて `## キャラクター設定\n\n### personality...\n### speaking_style...` のように囲む。
詳細な合成ロジックは `llm/プロンプトローダー.md` 側。
...
```yaml
parts:
  - paths: shared/response_generation/dynamic/character_info.j2
    lang: en
  - paths: shared/response_generation/dynamic/character_info.jp.j2
    lang: jp
...
- `llm/プロンプトローダー.md` — index.yaml を読み込んで部品を結合する実装
- `llm/コストとキャッシュ.md` — プロンプトキャッシュの設計考え方
