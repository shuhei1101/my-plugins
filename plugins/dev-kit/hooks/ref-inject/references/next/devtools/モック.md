...
export const handlers = [
  ...
  http.get("/api/v{N}/resources/:id", ({ params }) => {
    return HttpResponse.json({
      data: { id: params.id, name: "Mock Detail", canEdit: true },
    })
  }),

  http.post("/api/v{N}/resources", async ({ request }) => {
    const body = await request.json()
    return HttpResponse.json({ data: { id: "new-id" } }, { status: 201 })
  }),

  http.patch("/api/v{N}/resources/:id", async () => {
    return new HttpResponse(null, { status: 204 })
  }),

  http.delete("/api/v{N}/resources/:id", () => {
    return new HttpResponse(null, { status: 204 })
  }),
]
```
...
it("API 失敗時にエラー表示", async () => {
  server.use(
    http.get("/api/v{N}/resources", () => new HttpResponse(null, { status: 500 }))
  )
  // ... テスト
})
```
...
export const Empty: Story = {
  parameters: {
    msw: {
      handlers: [
        http.get("/api/v{N}/resources", () => HttpResponse.json({ data: [], meta: { totalRecords: 0, page: 1, pageSize: 20 } })),
      ],
    },
  },
}
...
test("API 500 のときエラー画面", async ({ page }) => {
  await page.route("/api/v{N}/resources*", (route) => route.fulfill({ status: 500 }))
