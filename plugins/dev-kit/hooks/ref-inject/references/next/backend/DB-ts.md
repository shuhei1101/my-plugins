...
- 戻り値:
  ...
  - `deleteXxx` → void
- DB アクセスは **SQL Builder**（`db.select() / .insert() / .update() / .delete()`）を使う（`Drizzleスタイル.md`）
...
- 楽観的ロック詳細: `DB楽観的ロック.md`
- ハードデリート + 履歴パターン: `DB変更履歴.md`
- Drizzle の API 選択: `Drizzleスタイル.md`
- スキーマ定義: `DB-ID設計.md`, `DBタイムスタンプ.md`, `DBエナム.md`, `DBリレーション.md`
