...
トランザクション内では `Promise.all` 禁止（`DBトランザクション.md`）。
...
- `クエリ-ts.md`, `DB-ts.md` — 具体的な書き方
- `DBリレーション.md` — relations 定義
- `DBトランザクション.md` — トランザクション

## 禁止

- `sql.raw()` / 文字列結合の SQL（`` `SELECT * FROM ${table}` ``）— SQL Injection の温床
