...
SPA 的なフローならクライアント側 sign in / sign out も可（ただし Server Action 推奨）:
...
ただし Server Action での処理が推奨（プログレッシブエンハンスメント・CSRF 対策が楽）。詳細: `認証アクション.md`
...
- `認証コンテキスト.md` — Server 側の `getAuthContext()`
- `認証セットアップ.md` — Better Auth 全体設定
- `認証アクション.md` — Server Action（推奨）

## 禁止

- セッショントークンを手動取得（`useSession` 経由で取る）
