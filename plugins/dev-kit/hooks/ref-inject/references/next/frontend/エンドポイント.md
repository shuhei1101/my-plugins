...
const RESOURCES_API = `/api/v{N}/resources`             // PR135: v{N} バージョニング
...
- API URL は **`/api/v{N}/`** 配下（PR135 でバージョニング導入）
...
| `{FEATURE}_API_URL` | API ルート群（`/api/v{N}/{feature}/...`） |
...
`Object — フィーチャグループ` の例のように、小文字 `const`（`RESOURCES` / `RESOURCES_API`）はファイル内 private（export しない）で、URL 合成のためだけに使う。
...
```ts
const RESOURCES = `/resources`
const RESOURCES_API = `/api/v{N}/resources`
...
const res = await fetch("/api/v{N}/resources")
