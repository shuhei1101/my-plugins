...
`paths: ["fieldName"]` で shadcn `<FormMessage>` が該当フィールド下に表示。
...
- 配置は **`{feature}/form.ts`**（PR135、`[id]/edit/` 配下に置かない）
- 型は **`z.infer<>`** で導出（手書き禁止）。型名 `{Feature}FormType` / Schema 名 `{Feature}FormSchema`
- `defaultResourceForm` を export（new 画面で利用）
- 共通プリミティブは `app/(shared)/schema.ts` に集約（単一フィーチャ固有のものは置かない）
- エラーメッセージは日本語
- 同じ schema を `route.ts` と `actions.ts` で別定義しない（form.ts を共用）
