...
軽量チェックなら proxy で。詳細: `backend/プロキシ.md`

---

## View 画面の詳細

詳細: `frontend/詳細ページ-tsx.md`, `frontend/詳細スクリーン-tsx.md`

---

## Edit 画面の詳細

詳細: `frontend/編集ページ-tsx.md`, `frontend/編集スクリーン-tsx.md`
...
- `[id]/page.tsx` は **View 画面そのもの**（PR135、`view/` サブルート廃止。`view/` を作らず、redirect だけのプレースホルダにもしない）
- 編集は `[id]/edit/page.tsx`
- 権限ガードは Edit `page.tsx` の `redirect` で実装
- view/edit 共通の hook / components は **`[id]/` 直下** に置く。View 専用フィールドが要るときだけ View 専用フックを別途作る（共通の `[id]/` 直下に混ぜない）
- Edit 専用の hook / components は `[id]/edit/` 配下

## 関連 references

- `frontend/フィーチャーフォルダ.md` — フィーチャ全体構成
- `frontend/詳細ページ-tsx.md`, `frontend/詳細スクリーン-tsx.md`
- `frontend/編集ページ-tsx.md`, `frontend/編集スクリーン-tsx.md`
- `frontend/useQueryパターン.md` — 共通 hook
- `backend/プロキシ.md` — proxy ガード
