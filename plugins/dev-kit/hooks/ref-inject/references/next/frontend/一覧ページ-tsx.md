...
import { fetchResources, ResourceSearchParamsSchema } from "@/app/api/v{N}/resources/query"
...
- **async function**（Server Component を保つ。`'use client'` / `useState` / `useEffect` / `useQuery` は Client Screen 側へ分離）
- `params` / `searchParams` は **`await`** 必須（Next.js 16。await し忘れると型エラー）
- 型ヘルパー **`PageProps<'/resources'>`** を使う
- 取得関数は `app/api/v{N}/{resource}/query.ts` から直接 import（`fetch(/api/v{N}/...)` 経由しない）
...
詳細: `frontend/conventions/ルートファイル規約.md`

## エラー時

同階層の `error.tsx` で catch。詳細: `frontend/エラー-tsx.md`

## 関連 references

- `frontend/一覧スクリーン-tsx.md` — Client Screen 側
- `frontend/conventions/サーバーvsクライアント.md` — Server / Client 境界
- `backend/クエリ-ts.md` — fetchResources 実装
