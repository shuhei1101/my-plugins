<!-- This file is a Japanese mirror of ルート定義.md. When updating the English original, update this file too. -->
# fastapi/routes — ルーター実装パターン

> このファイルは `ルート定義.md` の日本語ミラーです。
...
`AppError` 系は `register_exception_handlers` で HTTP に変換される（`shared/エラー定義.md`）。
...
- `fastapi/スキーマ.md` — ChatRequest / ChatResponse の作り方
- `fastapi/認証とエラー.md` — Depends + exception_handler
- `fastapi/アプリケーション.md` — アプリ全体構成
- `architecture/レイアウト.md` — route.py の位置づけ
