<!-- This file is a Japanese mirror of design-基本方針.md. When updating the English original, update this file too. -->
# 設計原則の優先順位

> このファイルは `design-基本方針.md` の日本語ミラーです。
...
- ただし「3 箇所目以降のために」過剰に抽象化はしない（**3 度書いてから抽象化** の指針は `リファクタリング判断.md`）
...
- **D (Dependency Inversion)**: 高レイヤは低レイヤを **関数の型エイリアス** で抽象化（dev-kit Python は関数の型でやる、`architecture/TypeScriptスタイル適用.md` / `architecture/依存パッケージ管理.md`）

### 拡張性意識（YAGNI は強制しない）

- 「いずれ来そう」な拡張ポイントは **最初から関数の型で抽象化** しておく（後付けより安い）
- ただし「来るかわからない抽象化」を増やすのは控える（`@overload` / `Protocol` の量産は逆効果）
- 抽象化の閾値判断は `リファクタリング判断.md`

---

## クラスと関数の優先順位

dev-kit Python は **関数ファースト**（`architecture/TypeScriptスタイル適用.md`）。クラスは:
...
- `architecture/リファクタリング判断.md` — 共通化 / 抽象化 / 設定外出しの判断基準
- `architecture/TypeScriptスタイル適用.md` — 関数ファースト + 型エイリアスでの DI
