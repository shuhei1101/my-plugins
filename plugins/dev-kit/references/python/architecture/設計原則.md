...
- However, do not over-abstract "for the third occurrence and beyond" (the guideline of **abstract only after writing it three times** is in `リファクタリング判断.md`)
...
- **D (Dependency Inversion)**: higher layers abstract lower layers via **function type aliases** (dev-kit Python uses function types — see `architecture/TypeScriptスタイル適用.md` / `architecture/依存パッケージ管理.md`)
...
- The threshold judgment for abstraction is in `リファクタリング判断.md`

---

## Class vs Function Priority

dev-kit Python is **function-first** (`architecture/TypeScriptスタイル適用.md`). Use classes for:
...
- `architecture/リファクタリング判断.md` — judgment criteria for consolidation / abstraction / externalizing settings
- `architecture/TypeScriptスタイル適用.md` — function-first + DI via type aliases
