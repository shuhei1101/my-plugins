<!-- This file is a Japanese mirror of レイアウト.md. When updating the English original, update this file too. -->
# トップレベルレイアウト + feature 内構造

> このファイルは `レイアウト.md` の日本語ミラーです。
...
> プロンプトファイルは **プロジェクトルート直下の `prompts/`** に置く（`llm/プロンプト執筆.md` 参照）。
...
詳細は `scripts/Pythonスクリプト.md`。

---

## 関連ファイル

- `architecture/TypeScriptスタイル適用.md` — feature 内で使う型エイリアス + 関数のスタイル
- `architecture/コンポジションルート.md` — main.py の責務
- `architecture/依存パッケージ管理.md` — 依存方向
- `core/命名規則.md` — feature 内ファイル名の標準
