<!-- This file is a Japanese mirror of 並列処理.md. When updating the English original, update this file too. -->
# concurrency/parallelism — 並列処理

> このファイルは `並列処理.md` の日本語ミラーです。
...
| やりたいこと | API | 備考 |
|---|---|---|
| 多数の HTTP 並列 | `asyncio` + httpx.AsyncClient | `concurrency/非同期処理.md` 参照 |
...
- `concurrency/非同期処理.md` — IO バウンドの本命
- `performance/パフォーマンスチートシート.md` — ボトルネック特定の手順
