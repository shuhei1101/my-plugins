<!-- This file is a Japanese mirror of プロンプトローダー.md. When updating the English original, update this file too. -->
# llm/prompts-loader — プロンプトのローダー実装

> このファイルは `プロンプトローダー.md` の日本語ミラーです。

`prompts/index.yaml` を読み、部品ファイル群を Jinja2 でレンダリングして 1 つのプロンプトに組み立てる実装規約。
プロンプトの **書き方** は `llm/プロンプト執筆.md` 参照。
...
`プロンプト執筆.md` の「グループ化」を実装する場合、`build_prompt` を拡張:
...
かつ system 全体を 1 つの `cache_control` ブロックに包めるようになる（`llm/コストとキャッシュ.md`）。
...
- `llm/プロンプト執筆.md` — 部品の書き方・分割・index.yaml の構造
- `llm/コストとキャッシュ.md` — `cache_control` / 「上から積む」設計の根拠
- `architecture/コンポジションルート.md` — build_prompt を main.py で配線する流れ
- `shared/定数.md` — `PROJECT_ROOT` の定義
