...
For **how to write** prompts, see `llm/プロンプト執筆.md`.
...
When implementing the "Grouping" in `プロンプト執筆.md`, extend `build_prompt`:
...
and lets you wrap the entire system into a single `cache_control` block (`llm/コストとキャッシュ.md`).
...
- `llm/プロンプト執筆.md` — how to write parts / split them / structure of index.yaml
- `llm/コストとキャッシュ.md` — rationale for `cache_control` / the "stack from top" design
- `architecture/コンポジションルート.md` — flow of wiring build_prompt in main.py
- `shared/定数.md` — definition of `PROJECT_ROOT`
