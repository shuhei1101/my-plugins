...
The loader implementation is in a separate file (`llm/プロンプトローダー.md`).
...
Express in directory structure the rule **"static values on top, dynamic values on the bottom"** described in `llm/コストとキャッシュ.md`:
...
parts:
  - path: characters/akane/static/personality.md
    category: character
  - path: characters/akane/static/speaking_スタイル.md
    category: character
```

The loader wraps consecutive parts of the same category into something like `## Character Profile\n\n### personality...\n### speaking_style...`.
Detailed assembly logic is on the `llm/プロンプトローダー.md` side.
...
- `llm/プロンプトローダー.md` — implementation that reads index.yaml and concatenates parts
- `llm/コストとキャッシュ.md` — design thinking around the prompt cache
