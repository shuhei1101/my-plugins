...
Abstract LLM APIs (Claude / OpenAI / Gemini, etc.) **as functions** so they can be injected.
...
If multiple features take `AsyncChatFn` as an argument, swapping providers is transparent.
...
`make_openai_chat(...)` returns a fully wired `AsyncChatFn`.
...
Switching providers takes **one line of configuration**:
...
The signature of `generate_response` doesn't change.
...
To receive the response one token at a time:
...
See `concurrency/async.md` for async generators.

---

## Related files

- `llm/instructor.md` — Structured output with Pydantic
- `llm/exceptions-retry.md` — Exceptions + retry
- `llm/cost-cache.md` — Token management
- `llm/prompts.md` — Prompt file management
- `architecture/ts-style.md` — Design philosophy of function type aliases
