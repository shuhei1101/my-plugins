<!-- This file is a Japanese mirror of コストとキャッシュ.md. When updating the English original, update this file too. -->
# llm/cost-cache — トークン管理とコスト最適化

> このファイルは `コストとキャッシュ.md` の日本語ミラーです。
...
最適化前にトークン使用量をログ出力する（`llm/プロバイダー.md` の例参照）:
...
- `llm/プロバイダー.md` — トークン使用量ログの実装
- `llm/例外とリトライ.md` — rate-limit と並んでコストに影響
- `shared/ロガー.md` — 構造化ログでコスト集計
