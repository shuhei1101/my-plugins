<!-- This file is a Japanese mirror of シークレットと環境変数.md. When updating the English original, update this file too. -->
# secrets-and-env — シークレット / 環境 / 構造 / アセットの分離

> このファイルは `シークレットと環境変数.md` の日本語ミラーです。
...
詳細な読み込み方は `shared/設定.md`（`pydantic_settings.BaseSettings`）。
...
- `shared/設定.md` — `pydantic_settings` の使い方
- `shared/定数.md` — 計算済み定数の置き場
- `architecture/リファクタリング判断.md` — 設定外出し判断
