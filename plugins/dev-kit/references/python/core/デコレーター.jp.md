<!-- This file is a Japanese mirror of デコレーター.md. When updating the English original, update this file too. -->
# 推奨デコレータ + ハンドラーデコレータ

> このファイルは `デコレーター.md` の日本語ミラーです。
...
詳細は `llm/例外とリトライ.md`。同じ実装を `shared/decorators.py` に置けば LLM 以外でも使える。

### `@with_timeout` 実装例

`asyncio.timeout` のラッパー。詳細は `concurrency/非同期処理.md`。
...
- `core/型ヒント.md` — 型ヒント基本
- `shared/エラー定義.md` — 例外階層
- `llm/例外とリトライ.md` — `@with_retry` の本格実装
