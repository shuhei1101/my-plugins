<!-- This file is a Japanese mirror of 型ヒント.md. When updating the English original, update this file too. -->
# 型ヒント

> このファイルは `型ヒント.md` の日本語ミラーです。
...
Pydantic / dataclass / TypedDict / Protocol の使い分け表は `architecture/TypeScriptスタイル適用.md` の中心セクションで管理している。重複防止のためここには置かない。

---

## 関連ファイル

- `core/デコレーター.md` — 推奨デコレータ + ハンドラーデコレータ + `@overload`
- `architecture/TypeScriptスタイル適用.md` — DTO 使い分け表 / TypeScript 風スタイル
- `core/言語ルール.md` — 例外階層 / import 順
- `core/コメント.md` — フィールド説明の書き方
