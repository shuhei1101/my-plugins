...
グローバル設定は **`staleTime: 0`**（QueryClient 側、`frontend/query-client-setup.md`）。
...
詳細: `shared/error-client-handler.md`
...
- `frontend/use-mutation-pattern.md` — 書き込み hook
- `frontend/query-client-setup.md` — QueryClient 設定
- `backend/query-ts.md` — 取得関数（fetchXxx）
- `backend/client-ts.md` — client.ts の fetch wrapper

## 禁止

- `'use client'` 省略
- `useState` でサーバーデータを抱える
- `queryKey` に欠落（フィルタ変更でキャッシュが混ざる）
- View / Edit で同じデータの取得 hook を別々に持つ（共通化必須）
- staleTime を高く設定して古いデータを見せ続ける（必要なケースだけ）
