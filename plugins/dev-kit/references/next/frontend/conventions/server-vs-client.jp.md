<!-- This file is a Japanese mirror of server-vs-client.md. When updating the English original, update this file too. -->
...
- Server の関数を直接呼ぶ（API ルートを経由しない、`api/v1/{resource}/query.ts` の関数を import）
...
- `page.tsx` `layout.tsx` `loading.tsx` `not-found.tsx` は Server Component を第一選択
- `error.tsx` `global-error.tsx` は `'use client'` 必須
- `XxxScreen.tsx`（インタラクションを含む）は `'use client'`
- データ取得は Server Component で（`api/v1/{resource}/query.ts` の関数を直接 import）
- Server → Client にデータを渡す時は **シリアライズ可能な値** に限る
- Client tree の中で再度 Server に戻すことはできない（children として渡すのは可能）
- `'use client'` は可能な限り深い葉に置く
- `generateMetadata` `notFound()` `redirect()` `cookies()` 等は Server Component のみ
