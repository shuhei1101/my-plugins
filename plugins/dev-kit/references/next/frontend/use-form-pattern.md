...
- Zod schema は **フィーチャ直下の `form.ts`** から import（`frontend/form-ts.md`）
...
- `frontend/form-ts.md` — Zod schema 定義
- `frontend/form-component.md` — shadcn `<Form>` で UI 組み立て
- `frontend/edit-screen-tsx.md` — 完全な利用例

## 禁止

- 型を手書き（Zod から推論）
- `defaultValues` と `values` を両方使う
- dirty 判定を `JSON.stringify` で比較
- `reset(data)` を submit 成功後に呼ばない（dirty が落ちない）
