...
|---|---|---|
| `route.ts` | HTTP ハンドラ（GET/POST/PATCH/DELETE） | `route-ts.md` |
| `client.ts` | クライアント側 fetch ラッパー | `client-ts.md` |
| `service.ts` | ビジネスロジック + トランザクション境界 | `service-ts.md` |
| `db.ts` | INSERT / UPDATE / DELETE 関数群 | `db-ts.md` |
| `query.ts` | SELECT 関数群 + フィルタ Zod + 戻り値型 | `query-ts.md` |
| `dbHelper.ts` | ページング計算等の共通関数 | `db-helper-ts.md` |
...
mutation の第一選択は **Server Actions**（`actions.ts`、`actions-ts.md` 参照）。
`route.ts` は以下の場合に限定:
