...
name: update
description: |
  Sync the current project's .work/ directory with the latest workspace templates.
  Manual invocation only — use /workspace:update.
---

# workspace:update — Sync .work/ with Latest Templates

Applies the latest workspace templates to an already-initialized `.work/` directory.
...
3. If absent, tell the user to run `/workspace:setup` first and exit
...
1. Invoke the `/workspace:work-start` skill
