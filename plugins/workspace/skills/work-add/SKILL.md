...
# workspace:work-add — Create Worktree
...
1. If called with arguments (e.g. `refactor/rename-pr-to-branch`), parse them:
   ...
   - Legacy invocation `/workspace:work-add PR58 refactor/foo` is also accepted for back-compat: the
     `PR{N}` token is dropped and only the branch suffix is used.
