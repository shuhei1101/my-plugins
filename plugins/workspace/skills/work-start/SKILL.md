...
# workspace:work-start — Start a New Branch
...
> `{repo}-wt-{type}-{title}`. The PR document filename is derived from the branch by replacing
> slashes with hyphens (`feat/foo-bar` → `feat-foo-bar.md`). An internal numeric ID is still tracked
> in `index.yaml` for cross-reference (commits, archive metadata), but it does not appear in branch
> names, worktree paths, or PR document filenames.
...
2. **If enabled**: invoke `/workspace:work-add` with the branch name:
