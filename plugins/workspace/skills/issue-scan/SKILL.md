...
description: |
  ...
  "コードをスキャン", "イシューを探して", or invokes `/workspace:issue-scan` explicitly.
---

# workspace:issue-scan — Scan Codebase for Issues
...
the file against those references and records discovered problems via `/workspace:issue-save`.
...
1. Check whether `.work/issues/` exists
   - If not → prompt for `/workspace:setup` and stop
...
1. For each discovered problem, invoke `/workspace:issue-save`:
