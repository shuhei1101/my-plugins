...
## API 入出力には Pydantic を使う

**原則**: 型ヒント（typing）だけだと実行時のバリデーションが効かないため、以下のような「入出力の型安全性が重要な箇所」では Pydantic モデルを使う。

### Pydantic を使うべき箇所
- 外部 API へのリクエストボディ・レスポンスデータ
- LLM への入力プロンプト（構造化）と LLM からの出力（Instructor で構造化出力）
- 設定ファイル（YAML / JSON）からの読み込みデータ
- ファイル間で受け渡しするデータ（CSV / JSONL のレコード）
- ユーザー入力をパースするデータ
- スレッド・プロセス間で受け渡すイベントデータ

### typing で十分な箇所
- 関数の引数・戻り値の型ヒント（ローカル処理）
- 内部ロジックで完結する dict / list の型表現
...
### 利点
- 実行時バリデーション（不正データを早期検知）
- IDE 補完・型チェック（mypy）が効く
- JSON Schema 自動生成（API ドキュメント生成にも使える）
- LLM の構造化出力（Instructor）との相性が抜群
