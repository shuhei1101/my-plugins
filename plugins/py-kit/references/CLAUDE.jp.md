...
| ファイル | 役割 |
|---|---|
| **`index.md`** (英語) / **`index.jp.md`** (日本語) | reference 一覧 + 1 行 description。注入時の description は英語版を使う（フックが parse する）。日本語版は人間が一覧確認するためのもの |
...
1. **`index.md`（または `index.jp.md`）** を読んで、各 reference の概要を把握
...
2. `index.md` から各 reference の description を引く
3. 各 reference 本文を読む
...
`py-kit:py-project` / `py-kit:py-script` 各スキルの Step 1 は、まずこの `index.md` を最初に読む。
...
- 新規 reference を追加したら **`index.md` + `index.jp.md` + `injection_rules.yaml`** の 3 ファイルを必ず更新する
- ファイル削除 / リネーム時も同様
- `references/CLAUDE.md` には、原則として「2 ファイルの役割を読め」以外の情報を書かない
  （個別 reference の説明は `index.md` のテーブルに集約）
