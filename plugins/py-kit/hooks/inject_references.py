...
注入の重複は「パターン単位の TTL トークン」で制御する:
    ~/.claude/tokens/py-kit/{session_id}.yaml
は pattern をキーにした YAML マップで、各エントリに injected_at (epoch 秒) を持つ。
`now - injected_at < TTL` の間は再注入しない。TTL はデフォルト 3600 秒、
環境変数 PY_KIT_INJECTION_TTL (秒) で上書きできる。
...
環境変数 PY_KIT_INJECTION_LANG=jp で index.jp.yaml + injection.jp.md.j2 に切替。
...
PLUGIN_NAME = "py-kit"
ENV_PREFIX = "PY_KIT"
LOG_TAG = "py-kit-references-injection"
DEFAULT_TTL = 3600
