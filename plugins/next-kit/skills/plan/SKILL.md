...
description: |
  ...
  Reads next-kit references and outputs a structured planning document (file tree, per-file roles,
  ...
  or invokes `/next-kit:plan` explicitly.
  Do NOT trigger for implementation tasks — use `next-kit:implement` instead.
---

# next-kit:plan — Implementation Plan Generator

Reads next-kit references and outputs a structured planning document for a
...
- この計画を確認後、`/next-kit:implement` で実装を開始
...
- Do NOT call `/next-kit:implement` automatically — the user reviews the plan first
