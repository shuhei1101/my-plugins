...
日本語が必要な場合は環境変数 NEXT_KIT_INJECTION_LANG=jp を設定すれば
...
def _eprint(msg: str) -> None:
    ...
    sys.stderr.write(f"[next-references-injection] {msg}\n")


def _plugin_root() -> pathlib.Path:
    """plugin ルート (plugins/next-kit) を返す。
...
def main() -> int:
    ...
    # 言語選択: 環境変数 NEXT_KIT_INJECTION_LANG=jp で日本語版
    lang = os.environ.get("NEXT_KIT_INJECTION_LANG", "en").lower()
    ...
    # 一度だけ注入する。トークンは ~/.claude/tokens/next-kit/{session_id}-{patternhash} に置く
    # (空ファイルとして溜まるが自動削除はしない)。
    session_id: str = data.get("session_id", "default")
    token_dir = pathlib.Path.home() / ".claude" / "tokens" / "next-kit"
    required: list[str] = []
    optional: list[str] = []
