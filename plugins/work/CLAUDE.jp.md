...
| # | バージョン | 日付 | 概要 |
|---|---|---|---|
| 1 | 2.55.0 | 2026-05-31 | `issue-save` スキルを削除。イシューファイルのフォーマット・採番を新リファレンス `issues/イシュー記述ルール`（`.work/issues/ISSUE-*.md` 編集時に自動注入）へ移設。`issue-create`・`issue-scanner` はリファレンスに従い直接イシューファイルを書く |
