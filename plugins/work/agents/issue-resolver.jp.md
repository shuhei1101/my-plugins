...
4. **イシューを連携**（work:start Step 6）: ブランチ文書の `## 関連イシュー` テーブルに追加し、
   **メインリポジトリ**の `_index.yaml`（git 管理外・ワークツリーには無い）に対して
   `issue-tool.py set-status ... --status in_progress` と `issue-tool.py add-branch ... --branch {branch}`
   を実行して `status` と `branches` を更新する。イシューファイルは**フロントマターを持たない**ので
   連携のためにファイル内を編集することはない。
5. **初回コミット**: ブランチ文書のみ。
6. **実装**: 採用 `## 対応案` + `## 自由記述` の指示に沿って修正。ブランチ上で意味のある単位でコミット。
