...
| # | 完了 | 作業内容 |
|---|---|---|
| 1 | - | 未解決の質問を `## QA` に記録（このドキュメント） |
| 2 | - | （実装タスク：ブランチ固有の作業に置き換え） |
| 3 | - | `.work/notes/` のノートドキュメントを更新 |
...
**`## テスト`** — 手動テスト・スモークテストの実施記録。確認した内容・実測結果・判定（OK / NG）を記録する。
テストを実施しなかった場合はプレースホルダー行を 1 行残す。

**`## QA`** — Step 2 からの未解決の質問をここに QA-XXX エントリとして記録してください
（Step 8 で追加されます）。

**`## 参考ドキュメント`** — 関連ノートへのリンク。今は空のままにしておく。ノートのパスは Step 10（最終コミット）で追加されます。
...
### ステップ 8: `## QA` セクションに開いている質問を記録（ワークツリー内）
...
### ステップ 9: 最初のコミット — ブランチドキュメントを作成し、実装を開始

#### 処理

1. ワークツリー内の**ブランチドキュメントのみ**をコミット（ブランチ：`{branch}`）
   - この段階ではノートを含めない — ノートは最終 Step 10 でコミットする
...
実装コミットが中間に続く。
最終コミット（Step 10）はノートとブランチドキュメント更新でブランチを締める。
...
### ステップ 10: 最終コミット — ノートとブランチドキュメントを更新
...
3. 見つからない場合 → `ノート記述内容ルール.md`（`.work/notes/` 配下を編集すると自動注入される）に従って新しいノートを作成: 現在状態のみ・YAML frontmatter 無し・末尾に `## 変更履歴` テーブルを持つ固定テンプレート
