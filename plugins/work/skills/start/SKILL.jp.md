...
**`## 参考ドキュメント`** — 関連ノートへのリンク（Step 8 のノートはここに追加されます）。
...
3. 見つからない場合 → `notes-content-rules.md`（`.work/notes/` 配下を編集すると自動注入される）に従って新しいノートを作成: 現在状態のみ・YAML frontmatter 無し・末尾に `## 変更履歴` テーブルを持つ固定テンプレート
