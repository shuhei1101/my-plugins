...
description: |
  ...
  「引き継ぎ書を作って」「次のブランチをセットアップして」「ハンドオフして」「handoff して」
...
> スキル名は歴史的および CLI 互換性のため `pr-handoff` のままですが、ワークフローは
> ブランチ用語で動作するようになりました — `PR{N}` セマンティクスはありません。
> 「次ブランチ候補」テーブルはすべてのブランチドキュメントの `## 次ブランチ候補` セクションに記載されます。
...
先行ブランチの `## 次ブランチ候補` に転記されるため、そのブランチが完了して pr-handoff が再び実行されると、
...
- 依存後続候補は先行ブランチの `## 次ブランチ候補` に転記され、次の pr-handoff 実行で予約
...
- `feat/feature-A` が完了して pr-handoff が再び実行されると、feature-B が次の予約ブランチになり、
