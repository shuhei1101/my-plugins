...
name: issue-review
description: |
  未レビューのイシューを 1 件ずつ見て、「ユーザー回答欄」を埋める — `## 意思`（対応する/対応しない）を
  決め、`## QA` に回答し、`## 自由記述` に対応指示を書く。スマホ主用途: 各イシューの読みやすい要約を提示し、
...
**対応する / 対応しない / 後で** をユーザーに選ばせる。あわせてイシューの `## QA` に回答させ、
`## 自由記述` に対応指示を残させる。スマホ前提: スマホの SSH ではイシューファイルを開きづらいため、
生のファイル閲覧ではなく、コンパクトな要約を提示し `AskUserQuestion`（タップ）で回答を集める。

結果は各イシューの **`# ユーザー回答欄`** — `## 意思`・`## QA` 各エントリの `**回答**:`・`## 自由記述`
の `**回答**:` に書き込む。後で `work:issue-resolve` がその内容に基づいて動く。
...
- **未レビュー** = `## 意思` の `**回答**:` が空（または `# ユーザー回答欄` 自体が未記入）のイシュー。
  既に意思が記入済みのものはスキップ。`closed/` は無視。
- イシューファイルのフォーマット（フロントマター無し・2 分割）は `work-dir/イシュー.md`
...
3. **後で** の場合 → イシューは触らず（`## 意思` は空のまま）次へ。
4. **対応する / 対応しない** の場合：
   a. イシューに `## QA`（未回答 = `**回答**:` が空）があれば、各エントリを（`AskUserQuestion` 1 回
      あたり最大 4 件までバッチで）`回答候補` を選択肢として提示し、回答を集める。回答を書き戻す:
      該当 QA の `**回答**:` に記入する。`## 対応案` が複数ある場合は、ここで採用案を確定させる。
   b. `## 意思` の `**回答**:` に結論を記入する（例: 対応する / 対応しない。自由記述でよい）。
   c. ユーザーが自由記述の対応指示・理由を述べた場合（意思ステップの自由入力または追問で）、
      `## 自由記述` の `**回答**:` に書き込む（無ければ空のまま）。
5. 次のイシューへ。
