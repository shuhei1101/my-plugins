...
description: |
  ...
  イシューの QA に回答し、対応メモを記録する。スマホ主用途: 各イシューの読みやすい要約を提示し、
...
`## QA` に回答させ、対応メモを残させる。スマホ前提: スマホの SSH ではイシューファイルを開きづらいため、
生のファイル閲覧ではなく、コンパクトな要約を提示し `AskUserQuestion`（タップ）で回答を集める。

結果は各イシューの**フロントマター `decision`**（source of truth）と `## QA` / `## 対応メモ` に
書き込む。後で `work:issue-resolve` がその決定に基づいて動く。
...
4. **対応する / 対応しない** の場合：
   ...
   c. ユーザーが理由・追加指示を述べた場合（対応可否ステップまたは追問で）、イシューの `## 対応メモ`
      に書き込む（見出しが無ければ作成。空なら付けない）。
5. 次のイシューへ。
