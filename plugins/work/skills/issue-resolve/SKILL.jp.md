...
description: |
  ...
  `## 意思` が「対応する」系のイシューは `work:issue-resolver` サブエージェントへ委譲し、ブランチを切り、
  選んだマージ方式（対応する（自動マージ）→ 直接マージ、対応する（マージはAIに任せる）→ AI 判断）に応じて
  master へマージするかマージ待ち最終コミットで止める。「対応しない」のイシューは使い捨ての 1 イシュー
  専用ブランチでクローズし、同一起動内で即 master へマージする。`/loop` での実行を想定。トリガー: 「イシューを対応して」「イシューを消化して」
...
- **意思=対応する系 + status: not_started** → `work:issue-resolver` サブエージェント（1 イシュー 1
...
- **意思=対応しない** → 使い捨ての 1 イシュー専用ブランチで `wontfix` クローズ（ファイルは `closed/`
  ...
  ユーザーのマージを待つ。）
- **意思=未チェック**（未レビュー＝全チェックボックスが `- [ ]`）と **意思=対応する系 + status: in_progress**
  （対応中。別セッションの可能性）→ スキップ。

イシューのフォーマット（フロントマター無し・回答欄が先頭）／ライフサイクルは `work-dir/イシュー.md`
（自動注入）が規定する — それに従う。`## 意思` のチェックボックスは AI が解釈し、`対応する` で始まる行が
`[x]` なら肯定（`（自動マージ）`→`direct_merge: true`、`（マージはAIに任せる）`→AI 判断）、
`- [x] 対応しない` を否定とみなす。全て未チェック（`- [ ]`）なら未レビュー扱い。
...
4. 候補を上から走査する。各エントリのイシューファイルを開き `## 意思` のチェックボックスを読む：
   ...
   - `- [x] 対応する（自動マージ）` → ACCEPT アクション（Step 3）・`direct_merge: true`。
   - `- [x] 対応する（マージはAIに任せる）` → ACCEPT アクション（Step 3）・`direct_merge` は AI が判断。
   - 全て `- [ ]`（未チェック＝未レビュー）→ スキップ。
...
1. 委譲前に、**メインリポジトリ**の `_index.yaml` でイシューを in-progress にする（セッション横断ロック）：
   ...
   ```
2. **`direct_merge` の値を決定する** — チェックされた `## 意思` の変種が優先する:
   1. `- [x] 対応する（自動マージ）` → `direct_merge: true`（ユーザーの明示選択。常に直接マージ —
      `_index.yaml` のフィールドや下記ヒューリスティックは無視する）。
   2. `- [x] 対応する（マージはAIに任せる）` → AI が判断する:
      a. `_index.yaml` の ISSUE-{N} エントリに `direct_merge` フィールドがあればその値を使う。
      b. なければイシュー本文を分析する:
