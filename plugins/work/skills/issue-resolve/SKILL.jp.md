...
1. 委譲前に、**メインリポジトリ**の `_index.yaml` でイシューを in-progress にする（セッション横断ロック）：
   ...
   ```
2. このイシュー用に `work:issue-resolver` サブエージェント（エージェントタイプ `work:issue-resolver`）を
   **1 つ**委譲する。渡す情報: `ISSUE-{N}` の id とパス、確定した方針（`## 修正案` 採用案 +
   `## 対応メモ`）、そして**マージ待ち最終コミット**までブランチを進める指示（マージはしない）。
3. サブエージェントの返却時：
