...
description: |
  ...
  issue per invocation. Accepted issues are dispatched to a `work:issue-resolver` subagent that
  creates a branch and drives it to the merge-waiting final commit; rejected issues are closed on a
  shared `chore/rejected-issues` branch. Designed to run under `/loop`. Trigger when the user says
  "イシューを対応して", "イシューを消化して", "resolve issues", "issue-resolve", or invokes
  `/loop /work:issue-resolve` / `/work:issue-resolve` explicitly.
---
...
- **accept + not_started** → dispatch a `work:issue-resolver` subagent (one subagent per issue) that
  creates a branch via `work:start`, implements the fix, and stops at the **merge-waiting final
  commit** (merge is the user's call, separately).
- **reject** → close as `wontfix` on the shared `chore/rejected-issues` branch (file moved to
  `closed/`), accumulating there until the user merges that branch.
- **pending** (unreviewed) and **accept + in_progress** (being worked, possibly another session) →
  skipped.

The issue frontmatter / lifecycle is governed by `work-dir/イシュー.md` (auto-injected) — follow it.

---

## Overview

- **Prerequisite**: issues have been triaged with `work:issue-review` (they carry `decision`).
...
2. Glob `.work/issues/ISSUE-*.md` (exclude `closed/`), read each frontmatter (`decision`, `status`),
   sort ascending by issue number.
3. Walk the list top-down and select the **first** issue that is actionable:
   - `decision: reject` → REJECT action (Step 2).
   - `decision: accept` and `status: not_started` → ACCEPT action (Step 3).
   - Skip `decision: pending` (unreviewed) and `decision: accept` + `status: in_progress`.
