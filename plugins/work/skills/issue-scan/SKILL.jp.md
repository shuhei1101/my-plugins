...
description: |
  ...
  ref-inject の reference と照合しコードをスキャンし、発見を `.work/issues/` にイシューとして書き出す。
  メインエージェントはオーケストレーションのみ: スキャンブランチ作成・ID ブロック割り当て・
  サブエージェント起動を行い、その後メタデータを集約してインデックスを更新・コミット・master マージする。
...
各々が独自のコンテキストで動く。メインエージェントの仕事は: 観点を選ぶ → ID ブロックを割り当てる →
スキャナを並列起動 → 戻ってきたメタデータを集約 → インデックス更新 → コミット → マージ。

分析をメインのコンテキストから排除することが本スキルの目的: イシュー本文はサブエージェントが書く
ファイル内に存在し、オーケストレーターのコンテキストには決して入らない。
...
| 主体 | 担当 |
|---|---|
| **メインエージェント（本スキル）** | ブランチ・観点選択・ID ブロック割り当て・`_index.yaml` / `_index.archive.yaml` 更新・コミット・マージ |
| **`work:issue-scanner` サブエージェント** | ソース読み込み・reference 受領・問題発見・`ISSUE-{N}.md` ファイル書き出し |

サブエージェントはインデックスファイルに**触れず**コミットも**しない**。メインエージェントはソースを
**読まず**コードを分析**しない**。
...
1. 各観点に重複しない ID ブロックを割り当てる。`SLOT = 30` として、観点 `i`（0-indexed）には
   開始 ID `START_i = L + 1 + i * SLOT` を与える。各サブエージェントは発見に `ISSUE-{START_i}`、
   `ISSUE-{START_i + 1}`… を使う — ブロックは重複しないので並列書き込みでも衝突しない。
2. [subagent: parallel · await all] 各観点について `work:issue-scanner` サブエージェントを起動する
...
- 作成された全イシューのメタデータ統合（本文は既にディスク上 — コンテキストには無い）
...
1. 返ってきた各イシューメタデータを `_index.yaml` の `issues` に追記する:
   ...
   - id: ISSUE-{N}
     ...
     tags: [{tags}]
   ```
2. `_index.yaml` の `last_id` を `L + N * SLOT` に設定する（予約レンジ全体を消費済みとする;
   並列スキャンのため ID は連続しない場合がある — これは想定内で無害）。
...
- メインエージェントはプロジェクトのソースもイシュー本文も読まない — サブエージェントのメタデータのみ。
- 並列スキャンのため ID は意図的に連続しない（各サブエージェントが予約ブロックを保持する）。
