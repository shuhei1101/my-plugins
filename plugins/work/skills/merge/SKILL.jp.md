...
- ステップ3が完了していること
- `WORK_MERGE_CONV2CLAUDE` が `false`/`0`/`no`/`off` でないこと（デフォルト: 有効）; 無効の場合 → ステップ5へスキップ
...
- `WORK_MERGE_AUTO_HANDOFF` が `false`/`0`/`no`/`off` でないこと（デフォルト: 有効）; 無効の場合 → このステップをスキップしてステップ13へ進む
