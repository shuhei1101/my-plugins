...
- コンフリクトなし → 次に進む
- コンフリクトあり → 以下を厳守する:
  - `git -X ours` / `git -X theirs` などの自動解消オプションは絶対に使わない
  - `git checkout --ours` / `git checkout --theirs` も使わない
  - `DD`（両親で削除）/`DU`/`UD`/`AA` のいずれかが含まれる場合は **絶対に自動解消せず、ユーザーに状態を報告して停止する**（master 側にだけ存在する追加ファイルを誤って削除する事故の原因になるため）
  - サブエージェントを起動してコンフリクト解消を委譲してはならない
  - 解消する場合も `git status -s` の全行をユーザーに見せ、master 側で追加されたファイル（`A` 系）が削除候補に紛れていないかを必ず確認する
  - 不明な点が 1 つでもあれば停止してユーザーに聞く
