...
1. `Write` `.work/issues/ISSUE-{N}.md`。イシューファイルの構造は **`references/work-dir/イシュー.md`** から自動的に注入されます
   （`.work/issues/` の下にファイルを書き込むと、その時点で注入されます）。注入されたテンプレートから作成してください — 呼び出し元から受け取ったフィールドを埋めてください。データが提供されなかった場合は `Scan scope` の行、`ユーザーの言葉` / `修正案` のセクションを省略してください。
