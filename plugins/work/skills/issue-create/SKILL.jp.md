...
1. 確定した各イシューについて次の ID を採番し（`last_id + 1` を順にインクリメント）、
   ...
   - **未解決事項があれば QA を提起する**（どの修正案を採るか、ユーザーが決めるべき設計上の選択など）:
     `## QA` に `QA-XXX` エントリ（状態: 未解決）を追加する。これは後で `work:issue-review` で回答される。
     未解決事項が無ければ `## QA` 見出しは付けない。
2. 全ファイルを書いたら、リファレンスに従って `_index.yaml` を更新する: 各イシューのエントリを
   `issues` に追記し（`type` / `priority` / `tags` / `scan_scope` / `status: not_started` をここに記録）、
