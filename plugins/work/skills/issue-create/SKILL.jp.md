...
`イシュー記述ルール` リファレンスが定める。これは `.work/issues/ISSUE-*.md` を書き込む際に
...
1. 確定した各イシューについて次の ID を採番し（`last_id + 1` を順にインクリメント）、
   ...
   - ファイル書き込み時に `イシュー記述ルール` リファレンスが自動注入される — **そのフォーマットに
     正確に従う**。本文は日本語: `# ISSUE-{N}: {タイトル}`・`**作成日**` 行・`## 問題`・任意の
     `## 修正案`。`Type` / `Priority` / `Tags` 行や `User's words` セクションは書かない。
2. 全ファイルを書いたら、リファレンスに従って `_index.yaml` を更新する: 各イシューのエントリを
   `issues` に追記し（`type` / `priority` / `tags` / `scan_scope` はファイルでなくここに記録）、
   `last_id` を使用した最大 ID に設定する。
...
- フォーマット・採番・index ルールは全て `イシュー記述ルール` リファレンスにある — ここで重複させない。
