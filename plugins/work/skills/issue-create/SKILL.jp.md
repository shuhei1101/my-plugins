...
1. 確定した各イシューについて次の ID を採番し（`last_id + 1` を順にインクリメント）、
   ...
   - **`# ユーザー回答欄`** では各選択肢を未チェックのチェックボックス（`- [ ]`）で事前記入する（例:
     `- [ ] 対応する` / `- [ ] 対応しない`）— ここでチェックを入れたり accept/reject を決めたりしない
     （後で `work:issue-review` でユーザーがチェックする）。`## 意思` を必ず置く。`## 自由記述` は無い。
   - **対応案が複数ある場合、または着手前に決めるべき判断がある場合は `## QA` を立てる。**
     各 QA は `### QA-N: {タイトル}` とし、**選択肢を要約**（`A) … / B) …`）、**`**推奨**:`** 行
     （AI がどれを推すか + 理由 1 行。「後で決める」禁止）、各選択肢を未チェックのチェックボックスで記入する
     （`- [ ] A`、`- [ ] B`）。`## 対応案` を複数挙げたときは「どの案で進めるか」を QA-1 として**必須**にする。
