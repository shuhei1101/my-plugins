...
1. 確定した各イシューについて次の ID を採番し（`last_id + 1` を順にインクリメント）、
   ...
   - **上半分（AI 記入欄）**を日本語で埋める。注入テンプレートのセクション（`## 概要` / `## 背景` /
     `## 現状` /（バグなら `## 原因`）/ `## 期待される状態` / `## 対応案` / `## 横展開` など）に従う。
     `Type` / `Priority` / `Tags` 行は書かない（`_index.yaml` に記録）。
   - **下半分（`# ユーザー回答欄`）**は見出しと**回答候補**だけを用意し、`**回答**:` は空のままにする
     （ここで accept/reject は決めない — 後で `work:issue-review` でユーザーが記入する）。
     `## 意思` / `## 自由記述` を必ず置く。
   - **対応案が複数ある場合、または着手前に決めるべき判断がある場合は `## QA` を立てる**
     （`QA-XXX` に `回答候補` を付け、`**回答**:` は空）。とくに `## 対応案` を複数挙げたときは
     「どの案で進めるか」の QA を**必須**にする。判断が不要なら `## QA` 見出しは付けない。
