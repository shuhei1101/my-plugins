...
and saves each as an issue file in `.work/issues/` via `/work:issue-save`.
...
1. For each confirmed issue, invoke:
   ```
   /work:issue-save --title "{title}" --type {type} --priority {priority} --tags {tags} --problem "{problem summary}" --user-words "{user's original input}" --fix "{suggested fix if known}"
   ```
