"""work references auto-injection hook.
...
注入の重複は「パターン単位 + リファレンスファイル単位」の二層 TTL トークンで制御する:
    ~/.claude/tokens/work/{session_id}.yaml
...
TTL はデフォルト 3600 秒、環境変数 WORK_INJECTION_TTL (秒) で上書きできる
(patterns / references 共通)。フック発火のたびに全セッションのトークンを走査し、期限切れ
エントリ (now >= expires_at) を削除する (空になったファイルは削除)。期限切れ後に再びマッチ
すれば再注入される。

WORK_INJECTION_DISABLE=true/1/yes/on で注入機構全体を停止できる (緊急停止用)。

description は references/_index.yaml (英語) から path -> description として取得する。
環境変数 WORK_INJECTION_LANG=jp で _index.jp.yaml + injection.jp.md.j2 に切替。
...
PLUGIN_NAME = "work"
ENV_PREFIX = "WORK"
LOG_TAG = "work-references-injection"
DEFAULT_TTL = 3600
...
def main() -> int:
    ...
    # 言語選択: 環境変数 WORK_INJECTION_LANG=jp で日本語版
