...
1. 依頼が **ソースコードをメインに編集するか** を判断する。
   - **しない**（調査・確認・リサーチ・コード読解・挙動の説明 — 「これどうなってる?」「X の状態は?」
     「Y を調べて」）→ `/work:quick-task` を実行。ブランチ + 調査結果を記録した軽量タスクドキュメントを
     作成する。
   - **する**（ソースコード実装）→ `/work:start` を実行してブランチを作成し、Step 2 へ進む。

#### 注記

- どちらの入口もブランチ + タスクドキュメントを作成する。違いは作業の性質（調査か、ソースコード実装か）と
  ライフサイクルの軽さ。
- `work:quick-task` の途中でソースコード実装になったら、同じブランチ上で `work:start` のライフサイクルに
  従って続行する。

##### 禁止事項

- `work:start` も `work:quick-task` も実行せずに編集・コミット
- master に直接コミット
