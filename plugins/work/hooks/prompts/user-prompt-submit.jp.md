...
### Step 2: ブランチドキュメントを読む前に QA をチェック
...
2. ブランチドキュメントを `.work/tasks/{YYMMDD}_{title}/{YYMMDD}-{日本語タイトル}.md`（タスクフォルダ内の唯一の `.md` ファイル）で読む
3. その `## QA` セクションに未解決エントリがあれば — **ここで止まる** — ユーザーにそれらを解決するよう要求。それ以上進まない
4. `## QA` がクリア（または空）ならば、同じブランチドキュメントの `## 作業内容` セクションを読む
