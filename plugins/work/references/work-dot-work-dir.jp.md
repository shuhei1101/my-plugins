...
**`_index.md`**: `.work/notes/` 配下の全ノートのカタログ。カテゴリ別に整理されている。**ノートを新規作成・リネームしたときは必ず `_index.md` も同じコミットで更新**し、該当カテゴリのエントリを追加・変更する。新規作成前に `_index.md` を読んで既存ノートを確認する。
...
3. 昇格済みのノートは削除し、同じコミットで `_index.md` のエントリも削除する
...
- **`notes/` 配下のノートを新規作成・リネームしたときは、同じコミットで `_index.md` を更新する**
- **昇格済みのノートを削除するときは、同じコミットで `_index.md` のエントリも削除する**
