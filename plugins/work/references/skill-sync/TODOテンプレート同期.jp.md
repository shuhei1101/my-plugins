...
| ファイルパス | 役割 |
|---|---|
| `plugins/work/scripts/setup-task.py` | ブランチドキュメントテンプレート（`_BRANCH_DOC_TEMPLATE` 定数） |
...
- [ ] `setup-task.py` の `_BRANCH_DOC_TEMPLATE` のセクション構成が `work-start` SKILL.md Step 7 の記入指示と一致している
...
- [ ] `_BRANCH_DOC_TEMPLATE` と SKILL.md Step 7 の記入ガイドが同期している
