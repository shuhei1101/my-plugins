...
| File path | Role |
|---|---|
| `plugins/work/scripts/setup-task.py` | Branch document template (`_BRANCH_DOC_TEMPLATE` constant) |
...
- [ ] Section structure in `_BRANCH_DOC_TEMPLATE` (in `setup-task.py`) matches the fill-in instructions in `work-start` SKILL.md Step 7
...
- [ ] `_BRANCH_DOC_TEMPLATE` and SKILL.md Step 7 fill-in guidance are in sync
